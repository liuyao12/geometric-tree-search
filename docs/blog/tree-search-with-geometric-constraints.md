...
We describe a general method, **Geometrically Constrained Tree Search (GCTS)**, to solve certain combinatorial problems of tree search with "geometric constraints", in some respect similar to methods in **geometric deep learning**, specifically *equivariant* neural networks. For illustration we shall focus on the tiling problem: given a set of tiles (in 2D or 3D), with or without local matching rules, determine if and how they tile the plane or space, or a subset with prescribed boundary. A useful summary is that **GCTS is memoization by way of encoding failures geometrically**: when a branch fails, its obstruction is recorded as a marking whose transformed overlap recognizes and rejects the same geometric situation later. The learned marking accelerates the tree search by identifying and excluding unwanted branches. When working, it shall compensate for a rudimentary (greedy and slow, no heuristics) tree search algorithm, and may provide an *interpretable* and *provably correct* algorithm. It shall become clear what is meant by all this.

There is a suggestive analogy here with Vaughan Jones's [planar algebras](https://arxiv.org/abs/math/9909027), in which operations are indexed by planar tangles and composition is literally geometric gluing. Terence Tao later emphasized this as a genuinely two-dimensional alternative to the usual one-dimensional strings of algebraic notation in his remembrance of [Vaughan Jones](https://terrytao.wordpress.com/2020/09/09/vaughan-jones/). GCTS similarly lets spatial composition carry the computation: transformed markings glue on overlaps, while incompatible overlaps encode a failed continuation. This is a family resemblance rather than an identification—our present construction has not been given the vector-space, multilinear, or planar-operad axioms of a planar algebra—but it suggests a useful question: whether failure certificates and their composition admit a natural planar-algebraic formulation.
