...
| Incremental voxel pair-corona SAT control, `scripts/solve_voxel_pair_corona.py` | Same finite unmarked corona and full viable-frontier question, with retained Glucose clauses and cached voxel overlap masks including outside-pool candidates. Positive witnesses and dead frontiers replay in the independent point verifier. Unknown budgets remain unresolved. Separate SAT scheduling; see [oracle report](projects/3d-point-corona-sat.md#incremental-voxel-sat-oracle). |
