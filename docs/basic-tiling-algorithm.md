...
| Finite point-corona PB/SAT research control, `scripts/solve_point_pair_corona.py` | Same unmarked point sums and viable-frontier existence question, with a proved finite seed-support candidate reduction. Uses different scheduling, not the reference graph lane. Independent witness and obstruction replay; trusted-solver UNSAT rather than an exported proof. See [local-label refinement](projects/3d-point-corona-sat.md). |
