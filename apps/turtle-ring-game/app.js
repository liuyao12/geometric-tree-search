...
function transformPlacement(placement, op, flipBack = false) { const visualSign = flipBack ? -1 : op.sym.planeSign; return {...placement, isReflected: (placement.isReflected || placement.orientation?.isReflected) !== (visualSign < 0), vertices: placement.vertices.map(p=>transformAffine(p, op)), occupancy: placement.occupancy.map(e=>({...e, point: transformAffine(e.point, op)})), marks: placement.marks.map(e=>({...e, point: transformAffine(e.point, op), component: mapComponent(e.component, op.sym), value: e.value * op.sym.planeSign})), segments: placement.segments.map(s=>({...s, p1: transformAffine(s.p1, op), p2: transformAffine(s.p2, op), component: mapComponent(s.component, op.sym), value: s.value * visualSign}))}; }
