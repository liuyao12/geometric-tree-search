...
function transform(p,sym) { return sym.permutation.map(i => sym.sign*p[i]); }
...
function orientTile(verts, occ, stripes, sym, idx, name) { const vertices=verts.map(p=>transform(p,sym)); const occupancy=occ.map(e=>({...e, point:transform(e.point,sym)})); const marks=[]; const segments=stripes.map(seg=>{ const p1=transform(seg.p1,sym), p2=transform(seg.p2,sym), component=mapComponent(seg.component,sym), value=seg.value*sym.planeSign; segmentPoints(p1,p2,markReach).forEach(point=>marks.push({point,component,value})); return {p1,p2,component,value}; }); return {idx,name,vertices,occupancy,marks,segments}; }
const turtleOrientations = symmetries().map((s,i)=>orientTile(turtleVerts,turtleOcc,turtleStripes,s,i,'Turtle'));
const trefoilOrientation = orientTile(trefoilVerts,trefoilOcc,trefoilStripes,symmetries()[0],0,'Trefoil');
...
let view={scale:.72, x:canvas.width/2, y:canvas.height/2}, placements=[], coronas=[], gameMove=0;
...
function flipClicked(i){ if(i<0) return; const turtle=placements[i]; const halfTurn = p => [-p[0],-p[1],-p[2]]; const reflect = p => [p[1],p[0],p[2]]; const op = (gameMove++ % 2) ? reflect : halfTurn; trefoilOrientation.vertices=trefoilOrientation.vertices.map(op); trefoilOrientation.occupancy=trefoilOrientation.occupancy.map(e=>({...e,point:op(e.point)})); trefoilOrientation.marks=trefoilOrientation.marks.map(e=>({...e,point:op(e.point)})); trefoilOrientation.segments=trefoilOrientation.segments.map(s=>({...s,p1:op(s.p1),p2:op(s.p2)})); const label = gameMove % 2 ? 'half-turn' : 'reflection'; statusEl.textContent=`Applied ${label} with a neighboring turtle, then rebuilt the marked patch.`; buildPatch(); }
...
canvas.addEventListener('pointermove',e=>{ if(!dragging)return; view.x+=e.clientX-last.x; view.y+=e.clientY-last.y; last={x:e.clientX,y:e.clientY}; draw();});
