...
import {catalog,MODES,VERSION} from './model.js?v=2.2.3';
...
async function preview(){
  ...
  [geometryGroup,pointGroup,edgeGroup].forEach(clear);$('viewMeta').textContent='Preparing exact point data';$('coverage').textContent='No run yet';$('viewTitle').textContent='Tile geometry';refresh();const w=new Worker(new URL('./worker.js?v=2.2.3',import.meta.url),{type:'module'});worker=w;
...
function runWorker(c,action='search',strategy=null){
  return new Promise(resolve=>{
    const w=new Worker(new URL('./worker.js?v=2.2.3',import.meta.url),{type:'module'});worker=w;let done=false;
