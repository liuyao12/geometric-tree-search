...
import {catalog,MODES,VERSION} from './model.js?v=2.2.1';
...
function lock(value){busy=value;for(const id of ['run','suite','probe','tile','radius','seconds','seed','pairBudget','mirrors','import'])$(id).disabled=value;$('stop').disabled=!value;$('probe').disabled=value||!!models.preview?.slab;}
...
async function preview(){
  selectedView='tiling';document.querySelector('.viewer').hidden=false;document.querySelector('.viewer-foot').hidden=false;learningPreview.host.hidden=true;for(const k of Object.keys(learningStates))delete learningStates[k];const sequence=++previewSequence;worker?.terminate();worker=null;results={};series={};models={};custom=$('tile').value==='custom'?custom:null;
  ...
  [geometryGroup,pointGroup,edgeGroup].forEach(clear);$('viewMeta').textContent='Preparing exact point data';$('coverage').textContent='No run yet';$('viewTitle').textContent='Tile geometry';refresh();const w=new Worker(new URL('./worker.js?v=2.2.1',import.meta.url),{type:'module'});worker=w;
...
function runWorker(c,action='search',strategy=null){
  return new Promise(resolve=>{
    const w=new Worker(new URL('./worker.js?v=2.2.1',import.meta.url),{type:'module'});worker=w;let done=false;
    ...
    w.onmessage=({data:e})=>{
      ...
      if(e.type==='marking-learning'||e.type==='marking-learned'){
        ...
        if(e.type==='marking-learned')e.marking=remember3DMarking(models[c.mode],e.marking);
      ...
      if(e.type==='progress'||e.type==='result'){if(e.marking&&learningStates[c.mode]?.event.marking?.saved)e.marking.saved=learningStates[c.mode].event.marking.saved;results[c.mode]=e;(series[c.mode]??=[[0,0]]).push([e.stats.totalMs,e.verification?.covered??e.covered??0]);refresh();if(active===c.mode)renderPatch();}
