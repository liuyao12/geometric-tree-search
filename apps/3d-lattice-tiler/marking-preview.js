...
export class MarkingPreview{
 ...
 accept(event){if(!this.model)return;
  if(event.type==='marking-learned'){
   ...
   const c=event.counts??{};this.status.textContent=`${event.totalPairs??event.pairs??0} pairs · ${c.valid??0} valid · ${c.invalid??0} invalid · ${c.unresolved??0} unresolved · ${event.phase==='resume'?`continuing · ${event.retained} resolved labels retained · ${event.pairNodes} attempts per pair`:event.phase==='replay'?'checking saved pair evidence':event.phase==='pair'?'trying the next pair':event.phase==='refine'?'refining free values':event.phase==='update'?event.status: event.action??'filling the 1-corona'}`;
  }
