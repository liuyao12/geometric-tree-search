...
export class MarkingPreview {
 constructor(host) {
  ...
  this.canvas.onmousemove=e=>{const r=this.canvas.getBoundingClientRect();this.hover={x:(e.clientX-r.left)*800/r.width,y:(e.clientY-r.top)*300/r.height};this.draw();};
 ...
 accept(event) {
  ...
  if(event.type==='marking-learned') {
   ...
   if(this.evidence.length)this.selectPair();
  } else {
