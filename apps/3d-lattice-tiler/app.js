...
import {
  ...
  tileSpecs
} from "./engine.js?v=20260830-a2-sliced8-v242";
    ...
    if (periodicLane) {
      ...
      candidateResearchDetail.textContent = candidate.kind === "a2_sliced_alcove_census"
        ? candidate.screening.status === "periodic"
          ? `${candidate.description} Exact determinant-${candidate.screening.periodic_twelve_copy_certificate.determinant} quotient search found a replay-verified ${candidate.screening.motif_tiles}-copy translational tiling with period vectors ${candidate.screening.periodic_twelve_copy_certificate.period_vectors.map(vector => `(${vector.join(",")})`).join(", ")}. This conclusively removes the tile from the aperiodic-candidate shortlist and retains it as a large-domain non-polycube periodic control.`
...
function ensureSolverWorker() {
  ...
  solverWorker = new Worker(new URL("./solver-worker.js?v=20260830-a2-sliced8-v239", import.meta.url), { type: "module" });
...
function startGrowthBenchmark() {
  ...
  for (const mode of GROWTH_MODES) {
    const worker = new Worker(new URL("./growth-benchmark-worker.js?v=20260830-a2-sliced8-v239", import.meta.url), { type: "module" });
