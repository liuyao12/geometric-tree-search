      ...
      candidateResearchDetail.textContent = candidate.kind === "a2_layered_polyprism_census"
        ? candidate.screening.status === "periodic"
          ...
          : candidate.screening.census_stage?.startsWith("a2_layered_size8_layer_essential")
            ? `${candidate.description} It has layer profile ${candidate.morphology.layer_profile.join("–")} and ${candidate.morphology.distinct_cross_sections} distinct cross-sections. The exact weighted quotient census first rejects 4,529/4,940 shapes with one-copy certificates and another 405 with two-copy certificates. This candidate exhausts every HNF quotient through five copies—35, 155, 455, 651, and 1,085 bases respectively—with zero solver unknowns; its five-copy sparse multicover search visits ${candidate.screening.periodic_five_copy_exact_multicover_nodes.toLocaleString()} states.${candidate.screening.corona_completed_verified ? ` An independently replayed root corona uses ${candidate.screening.corona_root_patch_copies} copies.` : ` The first-corona solver remains undecided after ${(candidate.screening.corona_solver_timeout_ms / 1000).toFixed(0)} seconds.`} Larger periodic domains, further corona extension, and substitutions remain open, so it is a bounded-unresolved non-polycube candidate rather than evidence of aperiodicity.`
