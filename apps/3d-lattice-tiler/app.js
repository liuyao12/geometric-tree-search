...
import {
  ...
  tileSpecs
} from "./engine.js?v=20260827-a2-size9-v233";
      ...
      candidateResearchDetail.textContent = candidate.kind === "a2_layered_polyprism_census"
        ? candidate.screening.status === "periodic"
          ...
          : candidate.screening.census_stage?.startsWith("a2_layered_size9_directed")
            ? `${candidate.description} Its layer profile is ${candidate.morphology.layer_profile.join("–")}; all ${candidate.morphology.layer_count} occupied slabs have distinct cross-sections, and the profile is not invariant under layer reversal. The focused exact census contains ${candidate.screening.source_pool_size} such shapes. Exact two-copy quotient search certifies ${candidate.screening.periodic_two_copy_certificates} periodic and leaves ${candidate.screening.periodic_two_copy_survivors}; a first five-candidate four-copy probe finds one more periodic tile. This candidate exhausts all ${candidate.screening.periodic_four_copy_hnf_visited}/${candidate.screening.periodic_four_copy_hnf_total} four-copy HNF quotients with zero solver unknowns. Its independently replayed root corona uses ${candidate.screening.corona_root_patch_copies} copies. Larger periodic domains and radius-two extension remain open, so this is a bounded-unresolved non-polycube lead, not evidence of aperiodicity.`
