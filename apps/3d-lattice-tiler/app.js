...
import {
  ...
  tileSpecs
} from "./engine.js?v=20260821-polycube10-v122";
import {
  ...
  if (candidate) {
    ...
    if (periodicLane) {
      ...
      candidateResearchDetail.textContent = candidate.kind === "polycube_census"
        ? candidate.screening.census_stage === "volume10_through5_radius2"
          ? `${candidate.volume}-cube chiral polycube; its omitted enantiomer ${candidate.mirror_equivalent_id} is the same tiling-existence problem under reflection of all space. Exact HNF search exhausted all ${candidate.screening.periodic_hnf_candidates_exhausted.toLocaleString()} quotients through ${candidate.screening.periodic_hnf_max_motif_tiles} copies without a periodic certificate. Its radius-${candidate.screening.corona_completed_radius} patch is independently verified, while radius ${candidate.screening.corona_next_radius} remains unresolved after ${candidate.screening.corona_next_time_limit_ms / 1000} CPU-seconds, ${candidate.screening.corona_next_nodes.toLocaleString()} nodes, and ${candidate.screening.corona_nogood_prunes.toLocaleString()} exact nogood prunes. This is a focused GCTS stress candidate; no tiling or aperiodicity claim is made.`
...
function ensureSolverWorker() {
  ...
  solverWorker = new Worker(new URL("./solver-worker.js?v=20260821-polycube10-v122", import.meta.url), { type: "module" });
