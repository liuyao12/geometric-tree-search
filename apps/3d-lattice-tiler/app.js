      ...
      candidateResearchDetail.textContent = candidate.kind === "a2_layered_polyprism_census"
        ? candidate.screening.status === "periodic"
          ...
          : candidate.screening.census_stage?.startsWith("a2_layered_size8_layer_essential")
            ? `${candidate.description} It has layer profile ${candidate.morphology.layer_profile.join("–")} and ${candidate.morphology.distinct_cross_sections} distinct cross-sections. The exact weighted quotient census first rejects 4,529/4,940 shapes with one-copy certificates and another 405 with two-copy certificates. This candidate exhausts every HNF quotient through six copies—35, 155, 455, 651, 1,085, and 2,015 bases respectively—with zero solver unknowns; its six-copy sparse multicover search visits ${candidate.screening.periodic_six_copy_exact_multicover_nodes.toLocaleString()} states. An independently replayed root corona uses ${candidate.screening.corona_root_patch_copies} copies. A longer second-corona GCTS run retains ${candidate.screening.corona2_gcts_sound_clauses} sound obstruction clauses over ${candidate.screening.corona2_gcts_rounds} proposals, but does not exhaust the outer search. Direct substitution is excluded for all 49 planar/layer scale pairs from 2 through 8; connected two-copy metatile substitutions are excluded at scalar scales 2 and 3, and the complete connected three-copy screen excludes at least scale 2.${candidate.screening.three_copy_metatile_substitution_scales_exhausted.includes(3) ? " Scale 3 is also exhausted for this candidate." : " Its three-copy scale-3 screen is still running."} Larger periodic domains, second-corona completion, and more general substitutions remain open, so it is a bounded-unresolved non-polycube candidate rather than evidence of aperiodicity.`
