...
import {
  ...
  tileSpecs
} from "./engine.js?v=20260824-polycube10-v187";
import {
  ...
  if (candidate) {
    ...
    if (periodicLane) {
      ...
      if (candidate.kind === "polycube_census" && candidate.screening.corona_placement_cube_cegar_rounds) {
        candidateResearchDetail.textContent += ` Placement-cube CEGAR now continues SAT partition leaves automatically. Its first ${candidate.screening.corona_placement_cube_cegar_rounds} exact-41 rounds reject ${candidate.screening.corona_placement_cube_cegar_proposals_rejected}/${candidate.screening.corona_placement_cube_cegar_proposals_checked} radius-three proposals at radius four, then grow the independently replayed feedback to ${candidate.screening.corona_placement_cube_cegar_final_clauses} clauses and ${candidate.screening.corona_placement_cube_cegar_final_cells} next-ring cells. Fixed-value propagation before PB bit-blasting then revisits ${candidate.screening.corona_propagate_values_historical_singletons} historical singleton leaves: ${candidate.screening.corona_propagate_values_unsat_singletons} become exact UNSAT and the sixth yields a new 41-copy proposal. GCTS rejects it at radius four in ${candidate.screening.corona_propagate_values_continuation_nodes} nodes, raising the independently replayed portfolio to ${candidate.screening.corona_propagate_values_replayed_clauses} clauses. Nested compatible-placement cubes close ${candidate.screening.corona_nested_partition_unsat_leaves} more partition leaves and isolate one open path with ${candidate.screening.corona_nested_partition_required_placements} fixed placements. Exact count 41 remains open, so this is a reproducible hard-core benchmark rather than a classification.`;
      }
