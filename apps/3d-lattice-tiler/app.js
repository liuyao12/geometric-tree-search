import {remember3DMarking} from './marking-storage.js?v=20260921-catalog-audit';
import {MarkingPreview} from './marking-preview.js?v=20260921-catalog-audit';
...
import {
  ...
  tileSpecs
} from "./engine.js?v=20260921-catalog-audit";
...
function updateCandidateResearchPanel() {
  ...
  if (candidate) {
    ...
    if (periodicLane) {
      ...
      const clusterRule = candidate.screening.periodic_quotient_cluster_substitution;
      candidateResearchDetail.textContent = `${sizeLabel} · ${source}; the motif has ${candidate.screening.motif_tiles} tiles and period vectors ${candidate.screening.period_vectors.map(vector => `(${vector.join(",")})`).join(", ")}.${clusterRule ? ` Treating that quotient as a metatile gives a replay-verified scale-2 substitution into ${clusterRule.child_metatile_copies} translated quotient clusters (${clusterRule.expanded_tile_copies} tile placements in the doubled cell).` : ""}${candidate.mirror_equivalent_id ? ` Its omitted enantiomer ${candidate.mirror_equivalent_id} is tiling-equivalent by reflection of all space.` : ""} The six-lane Run benchmark deliberately does not import this certificate: the ${periodicLane === "isohedral" ? "Isohedral" : "Translational"} lane must rediscover a witness from the tile geometry alone.`;
      ...
      candidateResearchTitle.textContent = candidate.screening.status === "periodic"
        ...
        candidateResearchDetail.textContent = candidate.kind === "a2_sliced_size10_alcove_census"
          ? `${candidate.description} Its solid-angle profile across the ${candidate.morphology.layer_count} consecutive sections is ${candidate.morphology.layer_weight_profile.join("–")}. The complete size-ten census contains ${candidate.screening.source_pool_size.toLocaleString()} connected shapes under the proper fixed-lattice symmetry group, with no layer-profile filter. The arithmetically minimal three-copy determinant-five screen finds ${candidate.screening.three_copy_periodic_certificates.toLocaleString()} replayed periods and completely excludes all 31 HNFs for the other ${candidate.screening.three_copy_proper_survivors.toLocaleString()} shapes; reflection reduction afterward gives ${candidate.screening.three_copy_reflection_classes.toLocaleString()} reporting classes. This candidate completely excludes all determinant-ten quotients through six copies. At nine copies the longer pass exactly excludes ${candidate.screening.periodic_nine_copy_exact_negative_orbits} of ${candidate.screening.periodic_nine_copy_orbit_total} proper-A₂ orbit classes, covering ${candidate.screening.periodic_nine_copy_hnfs_exactly_excluded} HNF bases; ${candidate.screening.periodic_nine_copy_node_capped_orbits} hard orbits remain explicitly inconclusive at two million nodes each. Direct scalar self-substitutions at scales 2 through 8 are excluded in both symmetry models, as are complete connected two-copy alphabets at scales 2 and 3 and three-copy alphabets at scales ${candidate.screening.three_copy_substitution_exact_scales.join(" and ")}. The complete four-copy alphabets are also excluded at scale 2 in both symmetry models across ${candidate.screening.four_copy_substitution_parents_exhausted.toLocaleString()} symmetry-distinct parents, each with a replayed local obstruction. An independently replayed ${candidate.screening.corona_root_patch_copies}-copy root corona survives local extension. The capped nine-copy orbits, clusters of five or more, non-scalar substitutions, and extension beyond the first corona remain open. These are exact bounded screening facts, not evidence of aperiodicity.`
          : candidate.kind === "a2_sliced_palindromic_alcove_census"
          ? `${candidate.description} Its solid-angle profile across the ${candidate.morphology.layer_count} consecutive sections is ${candidate.morphology.layer_weight_profile.join("–")}. The complete size-nine census contains ${candidate.screening.complete_source_pool_size.toLocaleString()} connected shapes. This candidate comes from the ${candidate.screening.recovered_source_pool_size.toLocaleString()}-shape palindromic-profile stratum that the earlier ranking heuristic omitted. Exact quotient search certifies ${candidate.screening.recovered_two_copy_periodic_certificates.toLocaleString()} of that stratum at two copies and another ${candidate.screening.recovered_four_copy_additional_periodic_certificates} at four copies; ${candidate.screening.recovered_reflection_classes_through_four} reflection classes reach the six-copy screen, which removes another ${candidate.screening.recovered_six_copy_additional_periodic_classes} and leaves ${candidate.screening.recovered_reflection_classes_through_six}. This candidate now completely excludes all ${candidate.screening.periodic_eight_copy_hnf_total} determinant-12 HNF quotients through eight copies (${candidate.screening.periodic_eight_copy_orbit_representatives} proper-A₂ orbits) with zero solver unknowns, visiting ${candidate.screening.periodic_eight_copy_exact_multicover_nodes.toLocaleString()} exact states. Direct scalar self-substitutions at every integer scale from ${candidate.screening.direct_scalar_substitution_exact_scales[0]} through ${candidate.screening.direct_scalar_substitution_exact_scales[1]} are excluded for both proper and reflected copies by ${candidate.screening.direct_scalar_substitution_certified_negatives} replayed atomic obstructions. Complete connected two-copy metatile screens exclude scales 2 and 3, and complete connected three-copy screens exclude scales ${candidate.screening.three_copy_substitution_exact_scales.join(" and ")}, in both symmetry models. The complete four-copy alphabets are likewise excluded at scale 2 in both symmetry models across ${candidate.screening.four_copy_substitution_parents_exhausted.toLocaleString()} parents, each with a replayed atomic obstruction. A ${candidate.screening.corona_solver === "z3" ? "separate Z3 backend" : "bounded exact GCTS run"} finds an independently replayed ${candidate.screening.corona_root_patch_copies}-copy root corona. Ten-copy and larger periodic domains, ${candidate.screening.five_copy_substitution_certified_negatives ? (candidate.screening.five_copy_substitution_models.includes("reflected") ? "clusters of six or more" : "clusters of six or more and reflected five-copy clusters") : "clusters of five or more"}, non-scalar substitutions, and extension beyond the first corona remain open. These are exact bounded screening facts, not evidence of aperiodicity.`
        ...
        ? candidate.screening.status === "periodic"
          ...
          ? `${candidate.description} Its solid-angle profile across the ${candidate.morphology.layer_count} consecutive sections is ${candidate.morphology.layer_weight_profile.join("–")}; this is an affine-A₃ tetrahedral-cell union, not a polycube or a constant-cross-section prism. The original directed stratum contains ${candidate.screening.source_pool_size.toLocaleString()} shapes: exact two-copy periods remove ${candidate.screening.two_copy_periodic_certificates.toLocaleString()}, exact four-copy periods remove another ${candidate.screening.four_copy_additional_periodic_certificates.toLocaleString()}, and quotienting the remaining ${candidate.screening.periodic_survivors_through_four.toLocaleString()} by reflection gives ${candidate.screening.periodic_survivor_reflection_classes_through_four} classes. Exact six-copy search removes another ${candidate.screening.six_copy_additional_periodic_certificates}. A closure pass now includes all ${candidate.screening.palindromic_profile_source_pool_size.toLocaleString()} previously omitted palindromic-profile shapes: ${candidate.screening.palindromic_profile_two_copy_periodic_certificates.toLocaleString()} have exact two-copy periods, another ${candidate.screening.palindromic_profile_four_copy_additional_periodic_certificates} have four-copy periods, and six-copy screening leaves ${candidate.screening.palindromic_profile_reflection_classes_through_six} classes. Thus the complete size-nine census contains ${candidate.screening.complete_source_pool_size.toLocaleString()} shapes and ${candidate.screening.complete_periodic_survivor_reflection_classes_through_six} reflection classes survive through six copies. Among the first ${candidate.screening.focused_eight_copy_candidates} priority classes of the original stratum, exact eight-copy search removes ${candidate.screening.focused_eight_copy_periodic_certificates} more and leaves ${candidate.screening.focused_eight_copy_survivors}. This candidate exhausts all ${candidate.screening.periodic_eight_copy_hnf_total} determinant-12 HNF quotients through eight copies with zero solver unknowns, visiting ${candidate.screening.periodic_eight_copy_exact_multicover_nodes.toLocaleString()} exact states. A longer ten-copy pass finds no period and exactly excludes ${candidate.screening.periodic_ten_copy_exact_negative_orbits} of ${candidate.screening.periodic_ten_copy_bounded_orbit_total} HNF orbits (${candidate.screening.periodic_ten_copy_hnfs_exactly_excluded} of ${candidate.screening.periodic_ten_copy_hnf_total} HNFs); ${candidate.screening.periodic_ten_copy_node_capped_orbits} hard orbits remain inconclusive after ${candidate.screening.periodic_ten_copy_exact_node_limit.toLocaleString()} exact nodes each. An independently replayed root corona surrounds it with ${candidate.screening.corona_root_patch_copies} copies. Focused radius-two GCTS/CEGAR retains ${candidate.screening.radius_two_failure_clauses} sound obstruction clauses over ${candidate.screening.radius_two_rounds} cumulative proposals after ${Math.round(candidate.screening.radius_two_cumulative_milliseconds / 1000)} seconds, without exhausting the outer corona space. A symbolic affine-inequality certificate excludes direct scalar self-substitution at every integer scale s≥${candidate.screening.substitution_direct_scalar_integer_scales_excluded_from}, with proper or reflected copies. Across all ${candidate.screening.substitution_two_copy_census_candidates_exhausted} original-stratum six-copy survivors, every connected two-copy metatile parent is excluded at scales 2 and 3 in both symmetry models. For this leader, every connected three-copy parent is likewise excluded at scales 2 and 3 in both models, and all ${candidate.screening.substitution_four_copy_metatile_types_exhausted.toLocaleString()} proper-orientation connected four-copy parents are excluded at scale 2.${candidate.screening.substitution_four_copy_reflected_types_exhausted ? ` The reflected four-copy alphabet is also completely excluded across ${candidate.screening.substitution_four_copy_reflected_types_exhausted.toLocaleString()} parents.` : ""} Larger periodic domains, complete radius-two extension, ${candidate.screening.substitution_four_copy_reflected_types_exhausted ? "clusters of five or more" : "reflected four-copy or larger clusters"}, and non-scalar substitutions remain open. These are exact bounded exclusions, not evidence that the tile is aperiodic.`
          : candidate.alcoves?.length === 8
          ? `${candidate.description} Its solid-angle profile across the ${candidate.morphology.layer_count} consecutive sections is ${candidate.morphology.layer_weight_profile.join("–")}; this is an affine-A₃ tetrahedral cell union, not a polycube or a constant-cross-section prism. Of ${candidate.screening.source_pool_size.toLocaleString()} directed eight-alcove shapes, exact weighted quotient search finds ${candidate.screening.three_copy_periodic_certificates.toLocaleString()} replayed three-copy periods. It advances ${candidate.screening.three_copy_periodic_survivors.toLocaleString()} shapes to the determinant-eight six-copy screen, which finds ${candidate.screening.six_copy_additional_periodic_certificates.toLocaleString()} additional periods. The remaining ${candidate.screening.periodic_survivors_through_six} reduce to ${candidate.screening.periodic_survivor_reflection_classes} reflection classes; longer determinant-16 passes certify ${candidate.screening.periodic_twelve_copy_certificates} of those classes periodic, leaving ${candidate.survivor_count} research candidates. This candidate exhausts the three- and six-copy quotient sizes with zero solver unknowns, visiting ${candidate.screening.periodic_three_copy_exact_multicover_nodes.toLocaleString()} and ${candidate.screening.periodic_six_copy_exact_multicover_nodes.toLocaleString()} exact states respectively; the 12-copy negative screen remains incomplete. Its independently replayed root corona uses ${candidate.screening.corona_root_patch_copies} copies.${candidate.screening.radius_two_status === "radius2_witness" ? ` GCTS also finds an independently replayed radius-two patch of ${candidate.screening.radius_two_patch_copies} copies.${candidate.screening.radius_three_status ? ` Resumed radius-three GCTS retains ${candidate.screening.radius_three_first_corona_failure_clauses} exact first-corona obstruction clauses after ${Math.round(candidate.screening.radius_three_cumulative_milliseconds / 1000)} seconds, but remains ${candidate.screening.radius_three_stopped_by.replaceAll("_", " ")}.` : ""}` : ` Bounded alternate-corona GCTS retains ${candidate.screening.radius_two_failure_clauses} sound failure clauses${candidate.screening.radius_two_stopped_by ? ` before ${candidate.screening.radius_two_stopped_by.replaceAll("_", " ")}` : ""}, without exhausting the outer space.`} Complete cellular screens exclude direct scalar substitutions at scales ${candidate.screening.substitution_direct_scalar_scales_excluded[0]} through ${candidate.screening.substitution_direct_scalar_scales_excluded.at(-1)} and connected two- and three-copy metatile substitutions at scales 2 and 3, with proper and reflected orientations; the proper-orientation four-copy alphabet at scale 2 is also completely excluded across ${candidate.screening.substitution_four_copy_metatile_types_exhausted.toLocaleString()} symmetry-distinct parents.${candidate.screening.substitution_four_copy_reflected_scalar_scales_excluded?.length ? ` The reflected four-copy alphabet at scale 2 is likewise completely excluded for this candidate across ${candidate.screening.substitution_four_copy_reflected_types_exhausted.toLocaleString()} symmetry-distinct parents.` : ""} Exact boundary-normal certificates show that all ${candidate.screening.substitution_anisotropic_cellular_inflations_excluded} expansive integral anisotropic maps (pI+cJ) with transverse and layer scales through 8 leave the affine-A₃ alcove arrangement, so they cannot define cellular substitutions in this fixed complex. ${candidate.screening.substitution_four_copy_reflected_scalar_scales_excluded?.length ? "Larger reflected clusters" : "Reflected four-copy clusters"}, complete 12-copy periods, complete radius-two extension, clusters of five or more, and non-cellular substitutions remain open. These are screening facts, not evidence that the tile is aperiodic.`
        ...
        ? candidate.screening.status === "periodic"
          ...
          : candidate.screening.census_stage?.startsWith("a2_layered_size9_directed")
            ? `${candidate.description} Its layer profile is ${candidate.morphology.layer_profile.join("–")}; all ${candidate.morphology.layer_count} occupied slabs have distinct cross-sections, and the profile is not invariant under layer reversal. The focused exact census contains ${candidate.screening.source_pool_size} such shapes. Exact two-copy quotient search certifies ${candidate.screening.periodic_two_copy_certificates} periodic and leaves ${candidate.screening.periodic_two_copy_survivors}; a first five-candidate four-copy probe finds one more periodic tile. This candidate now exhausts all ${candidate.screening.periodic_six_copy_orbit_representatives_total} proper-A₂ orbit representatives covering ${candidate.screening.periodic_six_copy_hnf_covered.toLocaleString()}/${candidate.screening.periodic_six_copy_hnf_total.toLocaleString()} six-copy HNF quotients, with zero solver unknowns and ${candidate.screening.periodic_six_copy_exact_multicover_nodes.toLocaleString()} exact multicover nodes. Its independently replayed root corona uses ${candidate.screening.corona_root_patch_copies} copies. Radius-two GCTS retains ${candidate.screening.corona2_gcts_sound_clauses} sound obstruction clauses after ${candidate.screening.corona2_gcts_rounds} rounds without exhausting the outer corona space. Direct cellular substitutions are excluded for all ${candidate.screening.direct_layer_scale_pairs_exhausted} planar/layer scale pairs from 2 through 8, every connected two-copy metatile is excluded at scalar scales ${candidate.screening.two_copy_metatile_substitution_scales_exhausted.join(" and ")}, and the complete connected three-copy alphabets are excluded at scalar scales ${candidate.screening.three_copy_metatile_substitution_scales_exhausted.join(" and ")} (${candidate.screening.three_copy_metatile_types_exhausted_by_scale[2].toLocaleString()} parent types at each scale). Larger periodic domains, complete radius-two extension, and clusters of four or more remain open, so this is bounded evidence rather than evidence of aperiodicity.`
          : candidate.screening.census_stage?.startsWith("a2_layered_size8_layer_essential")
            ? `${candidate.description} It has layer profile ${candidate.morphology.layer_profile.join("–")} and ${candidate.morphology.distinct_cross_sections} distinct cross-sections. The exact weighted quotient census first rejects 4,529/4,940 shapes with one-copy certificates and another 405 with two-copy certificates. This candidate exhausts every HNF quotient through ${candidate.screening.periodic_exact_through} copies with zero solver unknowns; its deepest sparse multicover search visits ${(candidate.screening.periodic_seven_copy_exact_multicover_nodes ?? candidate.screening.periodic_six_copy_exact_multicover_nodes).toLocaleString()} states. An independently replayed root corona uses ${candidate.screening.corona_root_patch_copies} copies. A longer second-corona GCTS run retains ${candidate.screening.corona2_gcts_sound_clauses} sound obstruction clauses over ${candidate.screening.corona2_gcts_rounds} proposals, but does not exhaust the outer search. Direct substitution is excluded for all 49 planar/layer scale pairs from 2 through 8; connected two-copy metatile substitutions are excluded at scalar scales 2 and 3; connected three-copy substitutions are excluded at scales 2 and 3; and all ${candidate.screening.four_copy_metatile_types_exhausted_by_scale[2].toLocaleString()} connected four-copy metatiles are excluded at scale 2. Larger periodic domains, second-corona completion, clusters of five or more, and non-cellular substitutions remain open, so it is a bounded-unresolved non-polycube candidate rather than evidence of aperiodicity.`
            : `${candidate.description} Exact weighted HNF search exhausts every periodic quotient through ${candidate.screening.periodic_exact_through} copies: all ${candidate.screening.periodic_determinant14_hnf_bases_exhausted} determinant-14 four-copy bases and all ${candidate.screening.periodic_six_copy_hnf_visited}/${candidate.screening.periodic_six_copy_hnf_total} determinant-21 six-copy bases, with no solver unknowns. At eight copies, ${candidate.screening.periodic_eight_copy_orbits_checked}/${candidate.screening.periodic_eight_copy_orbit_total} proper-A₂ symmetry-orbit representatives are independently VIPR-certified negative with ${candidate.screening.periodic_eight_copy_solver_unknowns} unknowns; the rest remain open. The six-copy sparse GCTS visits ${candidate.screening.periodic_six_copy_exact_multicover_nodes.toLocaleString()} exact multicover states. An independently replayed root corona uses ${candidate.screening.corona_root_patch_copies} copies. Focused second-corona CEGAR rejects ${candidate.screening.corona2_first_states_rejected}/${candidate.screening.corona2_first_states_checked} distinct first-corona states.${candidate.screening.corona2_gcts_sound_clauses ? ` A longer ${candidate.screening.corona2_gcts_rounds}-round exact GCTS extension retains ${candidate.screening.corona2_gcts_sound_clauses} sound placement-subset obstruction clauses; these clauses reject whole families, not merely single patches.` : ""}${candidate.screening.substitution_scalar_scales_excluded?.length ? ` Independently replayed atomic exact-cover screens exclude scalar self-substitution at scales ${candidate.screening.substitution_scalar_scales_excluded.join(", ")}.` : ""}${candidate.screening.substitution_anisotropic_inflations_excluded ? ` They also exclude all ${candidate.screening.substitution_anisotropic_inflations_excluded} cellular anisotropic planar/layer scale pairs from ${candidate.screening.substitution_anisotropic_scale_range[0]} through ${candidate.screening.substitution_anisotropic_scale_range[1]}.` : ""}${candidate.screening.substitution_two_copy_metatile_scalar_scales_excluded?.length ? ` Complete connected two-copy mixed-metatile screens exclude scalar scales ${candidate.screening.substitution_two_copy_metatile_scalar_scales_excluded.join(", ")}.` : ""}${candidate.screening.substitution_three_copy_metatile_scalar_scales_excluded?.length ? ` Complete connected three-copy mixed-metatile screens exclude scalar scales ${candidate.screening.substitution_three_copy_metatile_scalar_scales_excluded.join(", ")}.` : ""}${candidate.screening.substitution_four_copy_metatile_scalar_scales_excluded?.length ? ` Complete connected four-copy mixed-metatile screens exclude scalar scale ${candidate.screening.substitution_four_copy_metatile_scalar_scales_excluded.join(", ")}. Cluster sizes five and above and non-scalar metatile inflations remain open.` : ""} The outer corona and most eight-copy period orbits are not exhausted, so this is a bounded-unresolved non-polycube benchmark—not evidence of aperiodicity.`
...
function ensureSolverWorker() {
  ...
  solverWorker = new Worker(new URL("./solver-worker.js?v=20260921-catalog-audit", import.meta.url), { type: "module" });
...
function startGrowthBenchmark() {
  ...
  for (const mode of GROWTH_MODES) {
    const worker = new Worker(new URL("./growth-benchmark-worker.js?v=20260921-catalog-audit", import.meta.url), { type: "module" });
