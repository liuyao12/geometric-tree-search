...
import {
  ...
  tileSpecs
} from "./engine.js?v=20260823-polycube10-v186";
import {
  ...
  if (candidate) {
    ...
    if (periodicLane) {
      ...
      if (candidate.kind === "polycube_census" && candidate.screening.corona_placement_cube_cegar_rounds) {
        candidateResearchDetail.textContent += ` Placement-cube CEGAR now continues SAT partition leaves automatically. Its first ${candidate.screening.corona_placement_cube_cegar_rounds} exact-41 rounds reject ${candidate.screening.corona_placement_cube_cegar_proposals_rejected}/${candidate.screening.corona_placement_cube_cegar_proposals_checked} radius-three proposals at radius four, then grow the independently replayed feedback to ${candidate.screening.corona_placement_cube_cegar_final_clauses} clauses and ${candidate.screening.corona_placement_cube_cegar_final_cells} next-ring cells. Depth-first partitioning localizes ${candidate.screening.corona_placement_cube_cegar_open_singletons} singleton timeout leaves. Exact count 41 remains open, so this is a reproducible hard-core benchmark rather than a classification.`;
      }
