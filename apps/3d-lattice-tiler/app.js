...
import {
  ...
  tileSpecs
} from "./engine.js?v=20260831-substitution-v268";
...
function updateCandidateResearchPanel() {
  ...
  if (candidate) {
    ...
    if (periodicLane) {
      ...
      if (["a2_sliced_size10_alcove_census", "a2_sliced_palindromic_alcove_census"].includes(candidate.kind)) {
        candidateResearchDetail.textContent += ` The complete connected two-copy metatile family is also excluded at scalar scales ${candidate.screening.two_copy_substitution_exact_scales.join(" and ")} in the proper and reflected models, exhausting ${candidate.screening.two_copy_substitution_parents_exhausted.toLocaleString()} symmetry-distinct parent checks for this tile. At scalar scale 2 the complete connected three-copy family is likewise excluded across ${candidate.screening.three_copy_substitution_parents_exhausted.toLocaleString()} replayed parent checks. Three-copy scales beyond 2 and clusters of four or more copies remain open.`;
      }
