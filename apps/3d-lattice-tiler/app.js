...
import {
  ...
  tileSpecs
} from "./engine.js?v=20260821-polycube10-v134";
import {
  ...
  if (candidate) {
    ...
    if (periodicLane) {
      ...
      if (candidate.kind === "polycube_census" && candidate.screening.corona_cegar_pair_final_constraints) {
        candidateResearchDetail.textContent += ` Pairwise next-ring coverability promotes ${candidate.screening.corona_cegar_pair_filtered_states_checked} further proposals to ${candidate.screening.corona_cegar_pair_filtered_continuation_nodes}-node aggregate continuation proofs and learns ${candidate.screening.corona_cegar_pair_final_constraints} symmetry-closed cell-pair obligations; ${candidate.screening.corona_cegar_pair_final_timeout_runs} solves with the full pair set remain timeout-inconclusive. A factored witness-CNF encoding uses ${candidate.screening.corona_cegar_pair_witness_cnf_choice_variables.toLocaleString()} local choices and exposes ${candidate.screening.corona_cegar_pair_witness_cnf_states_checked} additional 63-copy proposals; exact GCTS rejects them in ${candidate.screening.corona_cegar_pair_witness_cnf_continuation_nodes} aggregate nodes, while ${candidate.screening.corona_cegar_pair_witness_cnf_timeout_runs} diversified solves remain inconclusive. Lazy GCTS independently replays ${candidate.screening.corona_cegar_pair_lazy_subset_replays_verified}/${candidate.screening.corona_cegar_pair_lazy_states_checked} pair clauses, with a minimum ${candidate.screening.corona_cegar_pair_lazy_minimum_clause_size}-placement explanation blocking ${candidate.screening.corona_cegar_pair_lazy_candidate_pairs_blocked.toLocaleString()} compatible next-ring placement pairs in aggregate; the equal-node outer A/B run is neutral because it encounters only simpler immediate obstructions.`;
      }
    }
