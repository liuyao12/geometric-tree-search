...
function render(update){latest=update;const {lanes,running,target}=update;
 ...
 for(const id of LANE_IDS){const s=lanes[id]?.state,card=$('lane_'+id);card.setAttribute('aria-pressed',String(id===selected));
  ...
  $('laneMemory_'+id).textContent=s?.learning?`${count(s.learning.addresses)} marking points · ${count(s.learning.entries)} values`:id==='known'?'Infinite line geometry':'No marking table';
 }
 const next=lanes[selected]?.state;if(state!==next){state=next;view=null;$('pointInfo').textContent='';queueDraw();}
 $('modeInfo').textContent=selected==='learned'?'Starts empty. Learns point constraints from certified impossible pairs.':selected==='plain'?'Geometry, corner capacity, and the selected tiles’ local matching rule.':'Infinite Ammann lines enforce matching directly.';
