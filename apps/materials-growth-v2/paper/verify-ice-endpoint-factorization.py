...
cross-checks coupled results against the earlier frozen precheck. Does not
...
coords,cover_path,dictionary_path,selection_path,library_path,paired_path,result_path,output=sys.argv[1:]
...
for conf in corpus['configurations']:
    ...
    for item in row['registrations']:
        ...
        assert set(item['coupledMatches'])=={m['motif'] for m in prior[item['edge']]['matches']}
...
result=dict(scope=__doc__,resultHash=sha(result_path),pairedPrecheckHash=sha(paired_path),verifierHash=sha(__file__),
