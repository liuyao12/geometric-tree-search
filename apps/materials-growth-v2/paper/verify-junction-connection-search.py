...
inp,learned,junction_path,connection_path,folder,kernel,out=map(Path,sys.argv[1:])
...
connections=json.loads(connection_path.read_text());assert connections['junctionHash']==digest(junction_path)
checks=[]
for path in sorted(folder.glob('[0-9]-*.json')):
    ...
    assert run['connectionHash']==digest(connection_path)
    ...
    if result['junction']:
        for node in junctions['folds'][f]['nodes']:
            ...
            junction_checks+=1
    coupled_checks=0
    if complete and result['connectionMode']=='learned-connections':
        labels={n['point']:next(i for i,s in enumerate(n['states']) if set(s['candidates'])==chosen&{e['candidate'] for e in n['incident']}) for n in junctions['folds'][f]['nodes']}
        for edge in connections['folds'][f]['edges']:
            if edge['candidate'] in chosen:assert [labels[p] for p in edge['points']] in edge['allowedPresentStates'];coupled_checks+=1
    checks.append({'file':cfg['file'],'connectionMode':result['connectionMode'],'complete':complete,'selected':len(ids),'positiveSupportComponents':components,'coupledConnectionsChecked':coupled_checks,
                   'junctionLiftsChecked':junction_checks,'filledPoints':sum(v==r['capacity'] for v in totals.values())})
assert checks
result={'scope':__doc__,'junctionHash':digest(junction_path),'connectionHash':digest(connection_path),'summaryHash':digest(folder/'summary.json'),'checks':checks}
