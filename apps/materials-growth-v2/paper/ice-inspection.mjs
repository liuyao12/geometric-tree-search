...
async function init(){
  ...
  const names={baseline:'GCTS baseline · partial','support-rich':'GCTS ranked · partial','connected-oracle':'Separate solver · verified full witness','certified-proof':'Reference search + checked proofs · full'};
  try{
    const response=await fetch(new URL('ice-proof-inspection.json',import.meta.url));if(!response.ok)throw Error('Proof-state data unavailable');
    const extra=await response.json(),c=data.configurations.find(c=>c.id===extra.id);
    if(!c||c.atoms!==extra.atoms||c.firstFrameHash!==extra.firstFrameHash||JSON.stringify(c.sourceOrder)!==JSON.stringify(extra.sourceOrder))throw Error('Proof-state source mapping mismatch');
    if(JSON.stringify(coverage(extra.state,c.atoms))!==JSON.stringify(extra.state.totals)||!extra.state.totals.every(t=>t===2))throw Error('Proof-state coverage mismatch');
    c.states['certified-proof']=extra.state;policy.add(new Option('Reference search + checked proofs · full','certified-proof'));policy.value='certified-proof';
  }catch(error){root.dataset.proofStateError=error.message;}
  ...
  function updateAtoms(){
    ...
    q('[data-ice-method]').textContent=policy.value==='certified-proof'?'New proof-assisted study: 52 restarted searches, 71.75 s cumulative acquisition/search time; four support components. Complete finite registered filling, not connected growth.':'Historical controls: the full witness in the separate-solver lane was found by mixed-integer optimization, not the timed GCTS runs.';
  ...
  async function loadConfig(){
    ...
    for(const option of policy.options)option.disabled=!current.states[option.value];
    if(!current.states[policy.value])policy.value='connected-oracle';
