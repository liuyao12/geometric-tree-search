...
const [compiledPath,junctionPath,alternativePath,kernel,dest,connectionPath,searchFolder]=process.argv.slice(2);
...
const connections=JSON.parse(readFileSync(connectionPath));assert.equal(connections.junctionHash,hash(junctionPath));
...
for(const input of compiled.models){
 ...
 const edges=new Map(connections.folds[input.fold].edges.map(e=>[String(e.candidate).padStart(6,'0'),e]));let excludedCombinations=0;
 ...
 for(const base of input.candidates){
  ...
  function emit(index,marks){
   ...
   if(edges.has(base.id)){
    const edge=edges.get(base.id),labels=new Map(marks.map(m=>[Number(m.point),m.lo])),pair=edge.points.map(p=>labels.get(p));
    if(!edge.allowedPresentStates.some(([a,b])=>a===pair[0]&&b===pair[1])){excludedCombinations++;return;}
   }
 ...
 assert.equal(candidates.length+excludedCombinations,junctions.folds[input.fold].expandedCandidateCount);
 ...
 const searchPath=searchFolder&&`${searchFolder}/${input.fold}-learned-connections.json`;
 ...
 checks.push({file:input.file,baseCandidates:input.candidates.length,markedVariants:candidates.length,excludedCombinations,lifted});
}
writeFileSync(`${dest}/check.json`,JSON.stringify({scope:'Explicit scalar junction labels with learned class-pair connection restrictions; all recorded fillings lifted and checked by the native point-value verifier.',junctionHash:hash(junctionPath),connectionHash:hash(connectionPath),kernelHash:hash(kernel),checks},null,2),{flag:'wx'});
