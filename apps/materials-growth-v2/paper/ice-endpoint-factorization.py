"""Controlled endpoint-factorization ablation on all frozen selected ice covers.
...
def run(coordinates, cover_path, dictionary_path, selection_path, portable_path, output):
    ...
    for c in corpus['configurations']:
        ...
        if row['training']: assert row['coupledComplete'] and row['factorizedComplete']
        results.append(row)
    ...
    result = dict(scope=__doc__, sourceHashes=library['sourceHashes'], portableHash=sha(portable_path),
                  codeHashes={name:sha(Path(__file__).with_name(name)) for name in ['ice-endpoint-factorization.py','ice-portable-pairs.py','portable-cloud-markings.py','ice-junction-clouds.py']},
                  summary=totals, results=results, seconds=time.monotonic()-start,
                  limits='Positive precheck against observed selected covers only. Fixed base poses, unchanged radii, no search or blind growth. Factorization admits previously unobserved endpoint combinations. Developmental data are not an independent final holdout.')
