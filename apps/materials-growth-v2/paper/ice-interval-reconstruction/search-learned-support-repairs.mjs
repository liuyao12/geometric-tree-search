...
writeFileSync(out,JSON.stringify({poolHash:sha(poolPath),kernelHash:sha(kernelPath),runnerHash:sha(new URL(import.meta.url)),results,
 ...
 marking:pool.marking??null,
 initialization:pool.models.every(r=>r.model.initial.length===0)?'empty-placement-state':'supplied-fixed-prefixes',
 limits:'Reference finite point search on supplied target atoms and finite geometry proposals. Marking declaration and prefix counts explicitly reported; marked runs are restricted hypothesis tests. No feasibility-solver selection used. Not single-atom unbounded growth, not complete continuous poses, not a marking speedup certificate.'},null,2),{flag:'wx'});
