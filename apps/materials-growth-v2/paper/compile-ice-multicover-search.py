...
for r in library['trainingRegistrations']: training[r['id']].append(r)
clouds = []; cache = {}; models = []
def intern(value):
...
for fold, cid in enumerate(ids_string.split(',')):
    ...
    lifts = []
    if cid in training:
        lifts = [{'name': record.get('coverVariant', 'original'), 'selected': [lookup[r['edge'], r['motif']] for r in record['selected']]} for record in training[cid]]
