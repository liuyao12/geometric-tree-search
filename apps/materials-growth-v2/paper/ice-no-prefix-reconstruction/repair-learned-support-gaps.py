...
no_fixed=len(sys.argv)>6 and sys.argv[6]=='no-fixed'
...
report=dict(sourceHashes={p.name:sha(p) for p in [coordp,supportp,posep,checkp]},codeHash=sha(Path(__file__)),rows=rows,noFixed=no_fixed,releaseNeighbors=release_neighbors,selectedIds=sorted(selected_ids) if selected_ids is not None else None,
