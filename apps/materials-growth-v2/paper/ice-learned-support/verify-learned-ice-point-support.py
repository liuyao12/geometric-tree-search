...
coordp,dictp,libp,resultp,out=map(Path,sys.argv[1:])
...
for reg in library['trainingRegistrations']:
    ...
    for selected in reg['selected']:
        ...
        pose=dd[cid]['occurrences'][edge];R=np.asarray(pose['rotationRow']);tr=np.asarray(pose['translation'])
...
report=dict(resultHash=sha(resultp),codeHash=sha(Path(__file__)),rows=rows,retiredTypes=sorted(retired_types),
