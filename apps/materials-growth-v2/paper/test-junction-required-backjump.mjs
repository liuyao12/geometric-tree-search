...
for(let trial=0;trial<150;trial++){
 ...
 const Engine=requiredBackjumpClass(mandatoryFrontierClass(linearFrontierClass(branchExclusionClass(junctionMarkingClass(PointSearch,nodes,model)))),model,nodes),e=new Engine(model),root=JSON.stringify(e.semanticState());let terminal=false;
...
const mark=(point,value)=>({point,channel:'0',lo:value,hi:value});
const example={capacity:2,required:['0b','1a','2d'],candidates:[
 {id:'bbad',t:[{point:'0b',value:2}],m:[mark('h',0)]},
 {id:'bgood',t:[{point:'0b',value:2}],m:[mark('h',1)]},
 {id:'j0',t:[{point:'1a',value:1}],m:[mark('g',1),mark('h',0)]},
 {id:'k0',t:[{point:'1a',value:1}],m:[mark('g',1),mark('h',0)]},
 {id:'j1',t:[{point:'1a',value:1}],m:[mark('g',0),mark('h',1)]},
 {id:'k1',t:[{point:'1a',value:1}],m:[mark('g',0),mark('h',1)]},
 {id:'d0',t:[{point:'2d',value:2}],m:[mark('g',0)]},
 {id:'d1',t:[{point:'2d',value:2}],m:[mark('g',0)]}]};
const tables=[{point:'1a',incident:['j0','k0','j1','k1'],states:[['j0','k0'],['j1','k1']]}];
const Example=requiredBackjumpClass(mandatoryFrontierClass(linearFrontierClass(branchExclusionClass(junctionMarkingClass(PointSearch,tables,example)))),example,tables),s=new Example(example);
let outcome;for(let i=0;i<100;i++){outcome=s.advance();if(['complete','exhausted'].includes(outcome.kind))break;}
assert.equal(outcome.kind,'complete');assert(s.requiredParentPrunes>0);assert(s.placed.has('bgood'));
console.log(JSON.stringify({adversarialRequiredParentPrunes:s.requiredParentPrunes,completion:true}));
