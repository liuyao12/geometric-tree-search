...
exported=json.loads(modelp.read_text());assert len(exported['models'])==1
modelrow=exported['models'][0];modelid=modelrow['id']
candidate_ids={c['id'] for c in modelrow['model']['candidates']}
assert len(candidate_ids)==len(modelrow['model']['candidates'])
runs=[dict(id=modelid,enabled=True,selected=result['selected'],status=result['status'])] if loop else result['results']
assert [(r['id'],r['enabled']) for r in runs]==([(modelid,True)] if loop else [(modelid,False),(modelid,True)])
rows=[]
for run in runs:
