...
assert(['baseline','support-rich','interleaved','coupled-observed'].includes(ordering));
...
for(const [rowIndex,row] of data.models.entries()){
 ...
 const setup=performance.now(),e=makeFactorizedCloudSearch(row,source.clouds,{Engine:ordering==='coupled-observed'?CoupledEndpointSearch:ordering==='interleaved'?InterleavedFactorizedSearch:ordering==='support-rich'?SupportOrderedFactorizedSearch:DynamicFactorizedSupportSearch,partnerIndex});
