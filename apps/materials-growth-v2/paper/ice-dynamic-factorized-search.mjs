...
const [sourcePath,blocksPath,dest,indexPath,indexCheckPath]=process.argv.slice(2),hash=p=>createHash('sha256').update(readFileSync(p)).digest('hex');
...
for(const [rowIndex,row] of data.models.entries()){
 ...
 const setup=performance.now(),e=makeFactorizedCloudSearch(row,source.clouds,{Engine:DynamicFactorizedSupportSearch,partnerIndex});
