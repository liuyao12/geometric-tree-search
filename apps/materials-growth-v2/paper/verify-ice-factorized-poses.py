"""Verify positive pose witnesses against periodic coordinates and cloud values.
...
assert data['codeHash']==sha(Path(__file__).with_name('ice-factorized-pose-audit.py'))
for key,path in zip(['coordinates','cover','dictionary','selection'],paths[:4]):assert library['sourceHashes'][key]==sha(path)
assert baseline['portableHash']==sha(library_path)
...
for row in data['results']:
    ...
    old={r['edge']:r for r in bb[cid]['registrations']}
    assert [r['edge'] for r in row['rows']]==selected
    for r in row['rows']:
        occurrence=dc[cid]['occurrences'][r['edge']]
        assert r['savedPoseMatched']==old[r['edge']]['factorized']
        ...
        for w in r['witnesses']:
            ...
            template=dictionary['types'][occurrence['type']]
            ...
            ends=v.ends(dict(occurrence,permutation=perm),cv[cid]['components'],cv[cid]['componentPairs'][r['edge']])
            ...
            for side,item in enumerate(w['endpoints']):
                ...
                assert base['pairType']==occurrence['type'] and base['componentSites']==[sites for sites,_ in ends]
...
result=dict(scope=__doc__,poseHash=sha(pose_path),verifierHash=sha(__file__),endpointFitsChecked=count,
            ...
            limits='Positive witnesses only; no independent exhaustive pose coverage or unknown-as-impossible claim. Fixed assigned bases and developmental selected covers, not search or blind growth.')
