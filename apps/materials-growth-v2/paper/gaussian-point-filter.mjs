...
// Two geometry inventories per marking point are required: pair intersection
// then has an explicit midpoint witness. No physical potential is evaluated.
...
export function gaussianPointClass(Base,model,{enabled=true}={}){
  const pool=model.fields,cache=new Map(),inventories=new Map();
  ...
  for(const c of model.candidates)for(const m of c.fieldM){assert(pool[m.field]);if(!inventories.has(m.point))inventories.set(m.point,new Set());inventories.get(m.point).add(c.base);}
  assert([...inventories.values()].every(s=>s.size<=2),'Only two-inventory marking anchors supported');
  ...
  return class GaussianPointSearch extends Base{
    ...
    }this.fieldStats=stats;super.refresh(new Set(this.points.keys()));}
