...
const sourceHashes=Object.fromEntries(['ice-raw-library-search.mjs','portable-cloud-filter.mjs','linear-frontier-decision.mjs'].map(n=>[n,hash(new URL(n,import.meta.url))]));
function audit(e,model,enabled){
 ...
 for(const c of model.candidates)assert.deepEqual([...e.reverse.get(c.id)].sort(),c.t.filter(t=>graph.get(t.point)?.has(c.id)).map(t=>t.point).sort());
...
for(const input of data.models)for(const enabled of [true]){
 const model={...input.model,candidates:input.model.candidates.map(c=>({...c,cloudM:c.cloudM.map(m=>({...m,cloud:data.clouds[m.cloud]}))}))};
 const Engine=portableCloudClass(linearFrontierClass(PointSearch),model,{enabled});
...
writeFileSync(`${dest}/summary.json`,JSON.stringify({scope:'Raw-cloud learned library, coupled endpoints, all finite registered variants and shared inventory. Marked-only 30-second diagnostic using global dead/forced then earliest generation. No support preprocessing or dynamic complement propagation. Common-cloud witnesses required for completion; unknown is not impossible. Not complete continuous-pose search or blind growth.',modelHash:hash(modelPath),kernelHash:hash(kernelPath),sourceHashes,results},null,2),{flag:'wx'});
