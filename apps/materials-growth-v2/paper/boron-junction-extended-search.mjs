...
import {junctionMarkingClass,independentJunctionDomains} from './junction-marking-filter.mjs';
...
const sourceHashes=Object.fromEntries(['boron-junction-extended-search.mjs','junction-marking-filter.mjs','linear-frontier-decision.mjs','branch-local-exclusions.mjs'].map(n=>[n,hash(readFileSync(new URL(n,import.meta.url)))]));
...
for(const input of compiled.models.filter(x=>[2,5].includes(x.fold)))for(const junction of [true]){
 ...
 const Engine=linearFrontierClass(branchExclusionClass(junction?junctionMarkingClass(PointSearch,nodes,model):PointSearch));
 ...
 const result={fold:input.fold,file:input.file,junction,maxSteps:1000000,maxSeconds:300,steps,seconds:(performance.now()-start)/1000,setupSeconds,junctionDiagnostics:e.junctionDiagnostics?{...e.junctionDiagnostics}:null,
