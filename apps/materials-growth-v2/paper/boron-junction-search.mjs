...
const sourceHashes=Object.fromEntries(['boron-junction-search.mjs','junction-marking-filter.mjs','linear-frontier-decision.mjs','branch-local-exclusions.mjs'].map(n=>[n,hash(readFileSync(new URL(n,import.meta.url)))]));
...
for(const input of compiled.models)for(const junction of [false,true]){
 ...
 const result={fold:input.fold,file:input.file,junction,steps,seconds:(performance.now()-start)/1000,setupSeconds,junctionDiagnostics:e.junctionDiagnostics?{...e.junctionDiagnostics}:null,
