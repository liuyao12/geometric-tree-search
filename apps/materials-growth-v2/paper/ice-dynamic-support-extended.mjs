...
const sourceHashes=Object.fromEntries(['ice-dynamic-support-extended.mjs','portable-cloud-filter.mjs','linear-frontier-decision.mjs','dynamic-half-support.mjs'].map(n=>[n,hash(new URL(n,import.meta.url))]));
function audit(e,model,enabled){
 ...
 const alive=new Set([...graph.values()].flatMap(cs=>[...cs.keys()])),groups=new Map(model.supportGroups.map(g=>[g.point,g.atoms])),support=new Map(model.complementSupport.map(c=>[c.id,c.groups]));
 let changed=true;while(changed){changed=false;for(const id of [...alive])for(const g of support.get(id))if(totals.get(groups.get(g.point)[0])===0&&!g.partners.some(j=>alive.has(j))){alive.delete(id);for(const cs of graph.values())cs.delete(id);changed=true;break;}}
 for(const c of model.candidates)assert.deepEqual([...e.reverse.get(c.id)].sort(),alive.has(c.id)?c.t.filter(t=>graph.has(t.point)).map(t=>t.point).sort():[]);
...
for(const input of data.models.filter(x=>x.fold===0))for(const enabled of [true]){
 const model={...input.model,candidates:input.model.candidates.map(c=>({...c,cloudM:c.cloudM.map(m=>({...m,cloud:data.clouds[m.cloud]}))}))};
 const Engine=dynamicHalfSupportClass(portableCloudClass(linearFrontierClass(PointSearch),model,{enabled}),model);
 ...
 while(steps<1000000&&performance.now()-start<300000){
 ...
 const result={fold:input.fold,file:input.file,enabled,maxSeconds:300,maxSteps:1000000,setupSeconds,seconds:(performance.now()-start)/1000,steps,stats:{...e.stats},selected,
