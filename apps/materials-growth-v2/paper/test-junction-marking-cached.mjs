...
import {junctionMarkingClass,independentJunctionDomains} from './junction-marking-cached-filter.mjs';
...
for(let trial=0;trial<150;trial++){
 ...
 const Engine=linearFrontierClass(branchExclusionClass(junctionMarkingClass(PointSearch,nodes,model))),e=new Engine(model),root=JSON.stringify(e.semanticState());let terminal=false;
 const Reference=linearFrontierClass(branchExclusionClass(Original(PointSearch,nodes,model))),reference=new Reference(model);
 for(let step=0;step<10000;step++){
  states++;assert.equal(canon(e.graph),canon(independentJunctionDomains(e,model,nodes)));e.auditGraph();
  assert.deepEqual(e.semanticState(),reference.semanticState());assert.deepEqual(e.decision(),reference.decision());
  ...
  const result=e.advance();assert.deepEqual(result,reference.advance());
...
console.log(JSON.stringify({models:150,subsets,states,restrictedSolutions,completionAgreement:true,graphAgreement:true,rootRollback:true,originalLockstep:true}));
