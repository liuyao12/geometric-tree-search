...
writeFileSync(`${dest}/summary.json`,JSON.stringify({inputScope:data.scope,inputLimits:data.limits,sourceHash:hash(input),kernelHash:hash(kernel),codeHashes:Object.fromEntries(['ice-gaussian-search.mjs','gaussian-point-filter.mjs','linear-frontier-decision.mjs'].map(f=>[f,hash(new URL(f,import.meta.url))])),results,limits:'Finite registered point-search control; see input scope for supplied geometry. Thirty-second cooperative search budget; a single update may exceed it. Preprocessing, setup and audit not included. Radius developmental; independent field replay pending.'},null,2),{flag:'wx'});
