"""Ice VI all assigned-base registered placements, without a selected cover.

Finite dictionary registrations only, not complete continuous-pose enumeration.
Six unsupported assigned registrations are reported, not silently filled from
...
libp,transferp,coverp,dictp,out=map(Path,sys.argv[1:])
...
cid='c01400';row=next(r for r in dictionary['configurations'] if r['id']==cid);cv=next(r for r in cover['results'] if r['id']==cid)
