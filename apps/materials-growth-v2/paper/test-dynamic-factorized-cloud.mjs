...
const build=pool=>makeFactorizedCloudSearch(model,pool,{Engine:DynamicFactorizedSupportSearch});
...
const bad=makeFactorizedCloudSearch({...model,cloudRadius:0},crossed,{Engine:DynamicFactorizedSupportSearch});assert.equal(bad.advance().kind,'exhausted');
...
assert.throws(()=>makeFactorizedCloudSearch({...model,capacity:1},[cloud(0),cloud(0)],{Engine:DynamicFactorizedSupportSearch}),/half-weight/);
console.log(JSON.stringify({permutedCloudCompletion:true,rootRollback:true,signatureFalsePositiveRejected:true,twoSidedGuardPreserved:true,unknownNotPruned:true,invalidCapacityRejected:true}));
