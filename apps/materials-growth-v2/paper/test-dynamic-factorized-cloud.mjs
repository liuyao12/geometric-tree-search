...
const Engine=process.argv.includes('--relation')?RelationalEndpointSearch:process.argv.includes('--coupled')?CoupledEndpointSearch:process.argv.includes('--interleaved')?InterleavedFactorizedSearch:DynamicFactorizedSupportSearch;
...
if(Engine===RelationalEndpointSearch)model.allowedPairs=model.blocks.map(()=>[[0,0]]);
...
console.log(JSON.stringify({relational:Engine===RelationalEndpointSearch,coupled:Engine===CoupledEndpointSearch,interleaved:Engine===InterleavedFactorizedSearch,permutedCloudCompletion:true,rootRollback:true,signatureFalsePositiveRejected:true,twoSidedGuardPreserved:true,unknownNotPruned:true,invalidCapacityRejected:true}));
