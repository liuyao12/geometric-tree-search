...
source_path,blocks_path,output,proof_path=map(pathlib.Path,sys.argv[1:])
...
for row,new in zip(data['models'],filtered['models']):
    start=time.monotonic();proof=support.prune(row,source['clouds']);proofs.append(proof)
    ...
    report={'file':row['file'],'fold':row['fold'],'before':str(before),'after':str(after),'removedEndpointRecords':len(proof['removals']),'passes':proof['passes'],'trainingLiftsPreserved':len(new['trainingLifts']),'seconds':time.monotonic()-start};reports.append(report);print(json.dumps(report),flush=True)
filtered['supportPreprocessing']={'originalBlocksHash':digest(blocks_path),'moduleHash':digest(pathlib.Path(support.__file__)),'reports':reports,'scope':'Necessary signature complement pruning of a finite half-weight model; survivors retain original cloud values. Not new learned markings.'}
