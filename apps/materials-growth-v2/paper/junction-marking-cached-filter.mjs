  ...
  refresh(_changed){
   ...
   for(let cursor=0;cursor<queue.length;cursor++){
    ...
    for(const state of node.states){
     ...
     // Full refresh above recomputes every reason. Each subsequent elimination
     // calls updateCandidate, so this cache stays current throughout the closure.
     if(state.some(id=>!this.placed.has(id)&&this.reasons.get(id)))continue;
