...
const [input,kernel,out,resumePath,resumeCheckPath]=process.argv.slice(2),data=JSON.parse(readFileSync(input)),row=data.models[0],model=row.model;
const sha=p=>createHash('sha256').update(readFileSync(p)).digest('hex'),{PointSearch,verify}=await import(pathToFileURL(kernel));
const resume=resumePath?JSON.parse(readFileSync(resumePath)):null;
if(resume){
 assert(resumeCheckPath,'Independent proof check required to resume');const check=JSON.parse(readFileSync(resumeCheckPath));
 assert.equal(check.resultHash,sha(resumePath));assert.equal(check.modelHash,sha(input));assert.equal(resume.sourceHash,sha(input));assert.equal(resume.kernelHash,sha(kernel));assert.equal(check.verifiedCores,resume.cores.length);
 assert(Number.isFinite(resume.cumulativeSeconds??resume.secondsIncludingRestarts));
}
const cores=structuredClone(resume?.cores??[]),initialCoreCount=cores.length,rounds=[],start=performance.now();let status='budget-unknown',selected=[];
...
const secondsIncludingRestarts=(performance.now()-start)/1000;
writeFileSync(out,JSON.stringify({scope:'Restarted marked reference search with ordered, validated geometry-only proof composition. No selected cover or feasibility-solver witness supplied.',sourceHash:sha(input),kernelHash:sha(kernel),resumeHash:resumePath?sha(resumePath):null,resumeCheckHash:resumeCheckPath?sha(resumeCheckPath):null,initialCoreCount,totalRounds:(resume?.totalRounds??resume?.rounds.length??0)+rounds.length,cumulativeSeconds:(resume?.cumulativeSeconds??resume?.secondsIncludingRestarts??0)+secondsIncludingRestarts,codeHashes:Object.fromEntries(['ice-composed-core-loop.mjs','gaussian-point-filter.mjs','linear-frontier-decision.mjs','verified-geometry-cores.mjs','composed-geometry-core.mjs'].map(f=>[f,sha(new URL(f,import.meta.url))])),cores,rounds,status,selected,secondsIncludingRestarts,limits:'Developmental finite registered pool and radius. Static validated rules per restart; no in-stack rule mutation. Up to 40 additional restarts/120 seconds per segment, cooperative budget. Cumulative time retains prior acquisition but excludes parsing and independent audits. Independent proof/witness replay pending. Not learned material markings, continuous completeness or blind growth.'},null,2),{flag:'wx'});
