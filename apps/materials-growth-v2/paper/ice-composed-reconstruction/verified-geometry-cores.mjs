...
export function verifiedGeometryCoreClass(Base,model,cores){
 ...
 for(const core of cores){
  ...
  assert(!verified.some(prior=>prior.owners.every(g=>used.has(g))),'Redundant core contains an earlier exclusion');
  assert(![...geometry].some(([g,t])=>!used.has(g)&&t.some(v=>v.point===core.point)&&t.every(v=>(totals.get(v.point)||0)+v.value<=model.capacity)&&!verified.some(prior=>prior.owners.every(owner=>owner===g||used.has(owner)))),'Unverified geometric core');
  verified.push(core);
 }
