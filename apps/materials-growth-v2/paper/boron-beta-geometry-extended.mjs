...
const sourceHashes=Object.fromEntries(['boron-beta-geometry-extended.mjs','portable-cloud-filter.mjs','linear-frontier-decision.mjs'].map(n=>[n,hash(new URL(n,import.meta.url))]));
...
for(const input of data.models.filter(m=>m.fold===2))for(const enabled of [true]){
...
writeFileSync(`${dest}/summary.json`,JSON.stringify({scope:'Extended 300-second portable-cloud beta-B106 reconstruction diagnostic on geometry-only registrations. No matched extended control or continuous-pose completeness claim.',extended:true,modelHash:hash(modelPath),kernelHash:hash(kernelPath),sourceHashes,results},null,2),{flag:'wx'});
