...
if search_mode:
    ...
    for run in repair['results']:
        ...
        assert run['status']=='complete';entry=models[run['id']];selected=run['selected']
        ...
        rows.append(dict(id=run['id'],status='unmarked-filling-awaiting-replay',fixed=[entry['geometry'][k] for k in selected if k in entry['model']['initial']],added=[entry['geometry'][k] for k in selected if k not in entry['model']['initial']]))
...
for result in repair['rows']:
    if result['status']!='unmarked-filling-awaiting-replay':continue
    ...
    assert all(v==1 for v in totals),(result['id'],Counter(totals))
    rows.append(dict(id=result['id'],atoms=len(pos),placements=len(placements),fixedPlacements=len(result['fixed']),addedPlacements=len(result['added']),
                     fullyFilled=len(pos),maxPositionErrorAngstrom=maximum,duplicateTypeCorrespondences=sum(v-1 for v in maps.values() if v>1)))
