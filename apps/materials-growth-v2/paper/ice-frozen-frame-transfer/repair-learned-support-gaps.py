...
external_targets=len(sys.argv)>6 and sys.argv[6]=='external-targets'
no_fixed=len(sys.argv)>6 and sys.argv[6] in ['no-fixed','external-targets']
...
source_paths=[coordp,supportp,posep,checkp]
target_rows=check['rows']
if external_targets:
    targetp=Path(sys.argv[9]);source_paths.append(targetp)
    assert len({p.name for p in source_paths})==len(source_paths)
    corpus=json.loads(targetp.read_text())
    target_ids=[c['id'] for c in corpus['configurations']]
    assert len(set(target_ids))==len(target_ids)
    assert set(target_ids).isdisjoint(r['id'] for r in check['rows'])
    assert set(target_ids).isdisjoint(support['trainingFrames'])
    target_rows=[dict(id=cid,training=False,valid=False) for cid in target_ids]
...
for row in target_rows:
    if row['training'] or (row['valid'] and not no_fixed):continue
    ...
    for p in ([] if no_fixed else poses['poses']):
    ...
    residual=2-totals;sites=np.flatnonzero(residual);candidates=[];seen=set();leaves=0;truncated=False;cloud_cache={}
    for typ,(X,colors,w) in sorted(templates.items()):
        ...
        for origin in sites:
            ...
            if origin not in cloud_cache:
                ...
                cloud_cache[origin]=(Q,np.linalg.norm(Q,axis=1))
            Q,radii=cloud_cache[origin]
            # This is the existing pair-distance test against assigned anchor 0,
            # moved ahead of correspondence recursion; it adds no new rule.
            choices=[np.flatnonzero((species[sites]==col)&(residual[sites]>=weight)&(np.abs(radii-D[0,i])<=2*tol+1e-9)).tolist() for i,(col,weight) in enumerate(zip(colors,w))]
...
report=dict(sourceHashes={p.name:sha(p) for p in source_paths},codeHash=sha(Path(__file__)),rows=rows,noFixed=no_fixed,externalTargets=external_targets,releaseNeighbors=release_neighbors,selectedIds=sorted(selected_ids) if selected_ids is not None else None,
...
if poolout:
    with poolout.open('x') as f:json.dump(dict(sourceHashes=report['sourceHashes'],codeHash=report['codeHash'],models=pools,scope='Finite unmarked pools. No MILP selections. Incomplete continuous registration proposals.',noFixed=no_fixed,usedSuppliedPoses=not no_fixed),f)
