...
const [inputPath,learningPath,kernelPath,dest]=process.argv.slice(2);
...
function audit(e,model){
 ...
 for(const c of model.candidates){const incident=[];
  if(!selected.has(c.id)&&c.t.every(x=>totals.get(x.point)+x.value<=model.capacity)&&c.m.every(x=>!marks.has(x.point)||marks.get(x.point)===x.lo))
...
for(const [fold,c] of d.configurations.entries()){
 ...
 for(const marked of [false,true]){
  ...
  const e=new PointSearch(model),root=JSON.stringify(e.semanticState());audit(e,model);
...
writeFileSync(`${dest}/summary.json`,JSON.stringify({scope:'In-sample known-coordinate reference search; same full pool marked/unmarked; no periodic selection lock or connectivity cuts in search; no blind growth',kernelHash:hash(readFileSync(kernelPath)),results},null,2),{flag:'wx'});
