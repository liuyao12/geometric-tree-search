...
const [input,kernel,dest]=process.argv.slice(2),data=JSON.parse(readFileSync(input));
...
for(const row of data.models)for(const enabled of [false,true]){
 const model=row.model,Engine=gaussianPointClass(linearFrontierClass(PointSearch),model,{enabled});
 ...
 const result={id:row.id,enabled,setupSeconds,seconds,steps,stats:{...e.stats},fieldStats:{...e.fieldStats},status:last?.kind==='complete'?'complete-awaiting-independent-replay':last?.kind==='exhausted'?'exhausted-finite-pool':'unknown',selected,scalarComplete:basic.complete};
...
writeFileSync(`${dest}/summary.json`,JSON.stringify({sourceHash:hash(input),kernelHash:hash(kernel),codeHashes:Object.fromEntries(['ice-gaussian-search.mjs','gaussian-point-filter.mjs','linear-frontier-decision.mjs'].map(f=>[f,hash(new URL(f,import.meta.url))])),results,limits:'Known cover and poses, unknown decorations; finite registered point-search control only. Thirty-second cooperative search budget; a single update may exceed it. Preprocessing, setup and audit not included. Radius developmental; independent field replay pending.'},null,2),{flag:'wx'});
