...
const sourceHashes=Object.fromEntries(['boron-junction-mandatory-search.mjs','junction-mandatory-order.mjs','junction-marking-arc-filter.mjs','linear-frontier-decision.mjs','branch-local-exclusions.mjs'].map(n=>[n,hash(readFileSync(new URL(n,import.meta.url)))]));
...
for(const input of compiled.models.filter(x=>[2,5].includes(x.fold)))for(const junction of [false,true]){
 ...
 const Engine=junction?mandatoryFrontierClass(Scheduled):Scheduled;
 ...
 const start=performance.now();let steps=0,last,mandatoryPreferencePlacements=0;
 ...
 const result={fold:input.fold,file:input.file,junction,ordering:junction?'generation-tie mandatory preference':'candidate-count/id',maxSteps:1000000,maxSeconds:100,mandatoryPreferencePlacements,steps,seconds:(performance.now()-start)/1000,setupSeconds,junctionDiagnostics:e.junctionDiagnostics?{...e.junctionDiagnostics}:null,
