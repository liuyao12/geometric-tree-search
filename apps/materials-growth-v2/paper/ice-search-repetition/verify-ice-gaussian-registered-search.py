...
import sys
import numpy as np
coordp,coverp,dictp,libp,transferp,modelp,resultp,out=map(Path,sys.argv[1:])
...
assert [(r['id'],r['enabled']) for r in result['results']]==[('c01400',False),('c01400',True)]
rows=[]
for run in result['results']:
    ...
    full=bool(np.all(totals==2));assert full==run['scalarComplete'];maxfield=0.;bad=0;paircount=0
