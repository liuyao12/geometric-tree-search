"""Independent full-pool and final-state checks for raw-library ice searches."""
import hashlib
...
for source in data['models']:
    ...
    for enabled in [True]:
        ...
        r = raw['result']; certified = r['markingStatus'] == 'verified-common-values'
        complete, components = check_state(r['selected'], certified, raw['commonWitnesses'] if certified else None)
        ...
        if r['status'] == 'verified finite registered filling': assert complete and certified
...
out = {'scope': __doc__, 'modelHash': sha(model_path), 'portableHash': sha(library_path), 'kernelHash': sha(kernel_path),
       ...
       'limits': 'Coupled raw-cloud markings, full registered candidate pool only. Cloud witness membership checked with separate Python implementation; runtime graph/scheduler/rollback audits are recorded by the unchanged JS harness. No independent continuous-pose completeness or connected-growth claim.'}
