...
a.output.mkdir()
manifest={'policy':'interleaved','phases':['Ih','II','VI'],'budgetSecondsPerFrame':30,
          'parentManifestHash':digest(a.source/'manifest.json'),'configurations':read(a.source/'manifest.json')['configurations'],
          'orderingHash':digest(here/'interleaved-factorized-search.mjs'),
...
for phase in manifest['phases']:
    ...
    subprocess.run([a.node,str(here/'ice-dynamic-factorized-search.mjs'),str(source/'source.json'),str(source/'filtered.json'),
                    str(out),str(source/'index.json'),str(source/'index-check.json'),'interleaved'],check=True)
    ...
    assert summary['sourceHashes']['interleavedOrdering']==manifest['orderingHash']
