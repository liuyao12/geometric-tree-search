...
const [inputPath,learningPath,kernelPath,dest,mode,stepArg,secondsArg,ordering='id',priorPath,supportPath]=process.argv.slice(2);
...
const residual=true,exclusions=true;
const hash=x=>createHash('sha256').update(x).digest('hex');
const researchSourceHashes=Object.fromEntries(['boron-support-residual-search.mjs','residual-capacity-filter.mjs','linear-frontier-decision.mjs','branch-local-exclusions.mjs','certified-dead-point.mjs','composed-point-proofs.mjs'].map(name=>[name,hash(readFileSync(new URL(name,import.meta.url)))]));
...
const supportRaw=supportPath?readFileSync(supportPath):null,support=supportRaw?JSON.parse(supportRaw):null;
assert(support&&!nogoods);if(support){assert.equal(support.inputHash,hash(raw));assert.equal(support.learningHash,hash(learnRaw));researchSourceHashes['certified-zero-support.mjs']=hash(readFileSync(new URL('./certified-zero-support.mjs',import.meta.url)));}
...
function audit(e,model){
 ...
 for(const c of model.candidates){const incident=[];
  ...
  if(!selected.has(c.id)&&!e.staticCertifiedExclusions?.has(c.id)&&!e.branchBlocked?.has(c.id)&&!cutBlocked&&c.t.every(x=>totals.get(x.point)+x.value<=model.capacity)&&c.m.every(x=>!marks.has(x.point)||marks.get(x.point)===x.lo))
...
for(const [fold,c] of d.configurations.entries()){
 ...
 for(const marked of [false,true]){
  ...
  const supportRecord=support?.results[fold];if(supportRecord){assert.equal(supportRecord.fold,fold);assert.equal(supportRecord.file,c.file);}
  const staticExcluded=certifiedZeroSupport(model,supportRecord?.certificate);
  // Certificate is verified against the entire unmodified model before creating
  // this immutable, scoped legality layer. No arbitrary constraint is admitted.
  class SupportEngine extends PointSearch {
   reason(candidate){return super.reason(candidate)||(staticExcluded.has(candidate.id)?'certified-zero-support':null);}
  }
  const Engine=linearFrontierClass(branchExclusionClass(residualCapacityClass(SupportEngine)));
  const e=new Engine({...model,preference:ordering!=='id'?c=>scores.get(c.id):()=>0});
  e.staticCertifiedExclusions=staticExcluded;
  const root=JSON.stringify(e.semanticState());audit(e,model);
  ...
  const result={fold,file:c.file,marked,status:check.complete?'exact finite point-cover witness':last?.kind==='exhausted'?'exhausted finite pool':'budget-unknown',
   ...
   staticCertifiedExclusions:staticExcluded.size,supportHash:supportRaw?hash(supportRaw):null,
