...
export const SCORE_NORMALIZATION_SPECS = Object.freeze({
  ...
  "ionic-pair": spec("incremental q_i q_j / r pair geometry or declared-medium finite Coulomb with optional Born–Mayer A_ij exp(-r/rho_ij), optional Tang–Toennies-damped -C6 f6(r/rho_ij)/r^6 attraction, and exact emitted-site gradient", "supplied charge² / Å, electronvolt, or electronvolt/Å", "d_nn and selected finite reach; optional declared ε_r, reference A/rho/C6, frozen uniform or observed-contact-scaled species-pair policy, and Kelvin T", "geometry mode normalizes r by d_nn; finite pair energy resolves exact species-pair coefficients and optional damped dispersion, maps x=ΔU/(N_added k_B T) through -x/(1+|x|), maps force cancellation y=F_rms d_nn/(k_B T) through (1-y)/(1+y), and combines by equal mean", "bounded ionic-pair energy-gradient ordering coordinate"),
  ...
  "classical-capillarity": spec("conditional finite-nucleus Wulff-scale work increment", "joule per complete frozen action before reduction", "independently validated classical nucleation barrier", "evaluate delta[C_gamma s^(d-1) - delta_g V_0 s^d] at the frozen-center pre/post scales and apply tanh(-delta work/barrier)", "[-1, 1]"),
