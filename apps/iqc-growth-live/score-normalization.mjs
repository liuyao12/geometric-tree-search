...
export const SCORE_NORMALIZATION_SPECS = Object.freeze({
  ...
  "ionic-pair": spec("incremental q_i q_j / r geometry or declared-medium finite Coulomb with optional Born–Mayer repulsion, damped pair dispersion, and damped charge-induced-dipole energy", "supplied charge² / Å, electronvolt, electronvolt/Å, or induced dipole eÅ", "d_nn and selected finite reach; optional declared ε_r, reference A/rho/C6/alpha, frozen uniform or observed-contact-scaled species-pair policy, and Kelvin T", "geometry mode normalizes r by d_nn; finite energy resolves exact pair coefficients and optional many-body charge-field induction, maps x=ΔU/(N_added k_B T) through -x/(1+|x|), maps pair-force cancellation y=F_rms d_nn/(k_B T) through (1-y)/(1+y), and combines by equal mean; polarization force is explicitly omitted", "bounded finite-interaction ordering coordinate"),
