...
export const FINITE_POINT_CHARGE_PROVENANCE = Object.freeze({
  model: "finite open-boundary formal-point-charge Coulomb interaction",
...
export function incrementalFinitePointChargeElectrostatics(currentSites = [], addedSites = [], {
  ...
  added.forEach((site, index) => added.slice(index + 1)
    .forEach((neighbor) => accumulate(site, neighbor)));
  ...
  const score = -reducedThermalEnergyPerAddedSite
  ...
  return {
    ...
    scoreDefinition: "-x/(1+|x|), x = delta U / (N_added k_B T)",
    ...
    claimBoundary: "This is the pair interaction energy of supplied formal point charges in a declared uniform isotropic relative permittivity over one finite open-boundary crop. It omits periodic images, Ewald summation, polarization, charge transfer, self energy, short-range repulsion, dispersion, electronic structure, and solvent or reservoir response; it is a conditional model, not a validated material energy.",
  };
}
