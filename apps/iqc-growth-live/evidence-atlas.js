...
const PHYSICS_MAP = {
  ...
  kinetics: {
    status: "progress", label: "thermodynamics + kinetics", title: "Equilibrium habit, kinetic habit, spatial supply, and exact events are separate evidence layers",
    physical: "The portal distinguishes four quantities that are often visually conflated: interfacial free energy γ(n̂), steady normal interface velocity v(n̂) at one declared driving condition, spatially resolved net incorporation flux J(x,n̂) over one frozen interface, and candidate-resolved transition-state barriers/prefactors. Exact local handoffs bind each response to one structure, interface/frame, method, settings digest, boundary condition, and uncertainty contract. Reversible finite event histories may then audit local grand-canonical balance, observed rate cycles, competing finite pathways, and a finite ΔΩ(N) profile.",
    geometric: "Validated γ(n̂) builds an equilibrium Wulff envelope and may regularize finite-nucleus support mismatch. Independently validated v(n̂) builds a kinetic-Wulff envelope. A validated J(x,n̂) quadrature resolves nonuniform substrate supply, depletion, or shadowing and may rank the same exact frontier by compact spatial/normal flux contrast. All three abstain outside supplied coverage and expose matched candidate-set/rank audits. Candidate-specific barriers remain attached to exact action IDs rather than being inferred from any envelope or field.",
