...
import { A2_SLICED_SIZE7_CANDIDATES } from "../../assets/a2-sliced-size7-candidates.js?v=20260828-319";
...
const A2_SLICED_SCALE4_OPEN = A2_SLICED_SIZE7_CANDIDATES.filter((candidate) =>
  candidate.screening.three_copy_metatile_scale4_reflected_status
    !== "no_three_copy_metatile_scalar4_substitution");
...
const SYSTEMS = {
  ...
  a2: {
    ...
    metrics: [["layered size-8 census", "4,940 → 4"], ["sliced size-7 census", "1,112 → 8 focused"], ["size-7 radius-2 patches", "8 / 8 complete"], ["scale-3 / scale-4 screens", `${A2_SLICED_SCALE3_OBSTRUCTIONS.length}/${A2_SLICED_SCALE4_OBSTRUCTIONS.length} obstructed · ${A2_SLICED_SCALE3_OPEN.length}/${A2_SLICED_SCALE4_OPEN.length} open`]],
    ...
    evidence: [
      ...
      ["Cluster substitution obstructions", `${A2_SLICED_SCALE3_OBSTRUCTIONS.length} candidates · ${(A2_SLICED_SCALE3_PARENT_COUNT + A2_SLICED_SCALE4_PARENT_COUNT).toLocaleString()} scale-specific parent tests`, `Complete proper/reflected three-copy searches certify no substitution at scale three for ${a2SlicedShortIds(A2_SLICED_SCALE3_OBSTRUCTIONS).join(" · ")} and at scale four for ${a2SlicedShortIds(A2_SLICED_SCALE4_OBSTRUCTIONS).join(" · ")}. ${A2_SLICED_SCALE3_OPEN.length + A2_SLICED_SCALE4_OPEN.length ? "At least one focused scale-specific screen remains open." : "Every focused candidate has completed both bounded screens."} These results rule out sixteen scale-specific bounded cluster grammars, not every substitution or global tiling.`],
...
const CLAIMS = [
  ...
  ["proved", "Sixteen bounded A₂ cluster obstructions", `${A2_SLICED_SCALE3_PARENT_COUNT.toLocaleString()} scale-three and ${A2_SLICED_SCALE4_PARENT_COUNT.toLocaleString()} scale-four reflected three-copy parent types are exhausted across ${a2SlicedShortIds(A2_SLICED_SCALE3_OBSTRUCTIONS).join(" · ")} with no substitution. Each certificate excludes one scale-specific finite grammar only; none proves non-tiling or aperiodicity.`],
