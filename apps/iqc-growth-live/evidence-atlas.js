...
const PHYSICS_MAP = {
  ...
  kinetics: {
    ...
    physical: "Temperature, pressure, chemical potentials, free-energy differences, diffusion barriers, nucleation rates, phonons, and time-dependent disorder. Imported relaxation archives can compare geometry-only final, first, or pooled reference mismatch with same-run relative energy or residual-force RMS, test a fixed contact/angle/coordination surrogate by leaving out one frame at a time, freeze it for another provenance-compatible archive, and—only after a predeclared transfer gate—opt into a bounded candidate-ranking mark.",
    ...
    boundary: "The portal predicts structurally admissible continuation, not a growth rate or thermodynamic phase diagram. Final, first, and pooled references are geometry-only sensitivity choices. The fixed ridge surrogate fits source labels; cross-archive use freezes it and requires the same reduced composition, periodicity, program/version, exact canonical normalized method record, units, target, and reference. Target labels score only after prediction. Promotion additionally requires at least five target frames, Spearman rho at least 0.80, and positive predictive Q-squared; explicit opt-in then adds only a bounded soft rank term over identical frozen candidates. One compatible archive is still not broad validation, causality, a potential, a force field, kinetics, or time.",
    systems: [
