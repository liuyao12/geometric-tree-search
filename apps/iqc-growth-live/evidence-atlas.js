...
const PHYSICS_MAP = {
  ...
  kinetics: {
    status: "progress", label: "thermodynamics + kinetics", title: "Equilibrium habit, kinetic habit, and exact events are now separate evidence layers",
    physical: "The portal distinguishes three quantities that are often visually conflated: interfacial free energy γ(n̂), steady normal interface velocity v(n̂) at one declared driving condition, and candidate-resolved transition-state barriers/prefactors. Exact local handoffs bind each response to one structure, frame, method, settings digest, and uncertainty contract. Reversible finite event histories may then audit local grand-canonical balance, observed rate cycles, competing finite pathways, and a finite ΔΩ(N) profile.",
    geometric: "Validated γ(n̂) builds an equilibrium Wulff envelope and may regularize finite-nucleus support mismatch. Independently validated v(n̂) builds a kinetic-Wulff envelope and may rank the same exact frontier by signed angular velocity contrast. Both preserve polar orientation, abstain outside supplied angular coverage, and expose matched candidate-set/rank audits. Candidate-specific barriers remain attached to exact action IDs rather than being inferred from either envelope.",
    growth: "Tree search still leap-frogs between hard-certified structural states. Equilibrium-shape, orientation-speed, or exact event-rate evidence can order one immutable candidate catalog only after explicit validation; proposal checks and backtracks remain computational work, while a physical waiting time exists only for a complete finite HTST catalog with declared temperature and prefactors.",
    boundary: "One finite orientation set is not a complete equilibrium or kinetic habit, and one steady velocity field does not transfer across driving force, composition, temperature, or mechanism. Neither morphology nor γ supplies v. A kinetic-Wulff prior is not an attachment probability, action barrier, diffusion coefficient, complete kinetic law, or physical clock. Candidate-resolved HTST remains conditional on the enumerated catalog; missing states and mechanisms remain open, so the portal does not claim a universal growth rate or phase diagram.",
    systems: [
