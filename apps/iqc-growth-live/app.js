...
const solutePartitionWeightSelect = $("solutePartitionWeightSelect");
...
const defectPrecursorSelect = $("defectPrecursorSelect");
const defectPrecursorWeightSelect = $("defectPrecursorWeightSelect");
const defectPrecursorHint = $("defectPrecursorHint");
const coherencyMemorySelect = $("coherencyMemorySelect");
...
let acceptedSolutePartitionScore = 0;
let rejectedSolutePartitionScore = 0;
...
let acceptedDefectPrecursorScore = 0;
let rejectedDefectPrecursorScore = 0;
let acceptedDefectPrecursorBurden = 0;
let rejectedDefectPrecursorBurden = 0;
let defectPrecursorEvaluations = 0;
let acceptedCoherencyMemoryScore = 0;
let rejectedCoherencyMemoryScore = 0;
...
let defectPrecursorMode = "none";
let defectPrecursorWeight = .24;
...
const GROWTH_PROTOCOLS = Object.freeze({
  ...
  directional: {
    ...
    settings: { confinement: "box", geometryPreference: "strain", geometricStrainWeight: .16,
      compositionPreference: "soft", chargePreference: "auto", surfacePreference: "soft", growthDrivingMode: "volume-driven", attachmentTopologyMode: "step", habitAnisotropyMode: "axial", defectPrecursorMode: "suppress", coherencyMemoryMode: "continue",
      ...
      thermalFieldMode: "z-plus-cold", thermalFieldWeight: .24,
      solutePartitionMode: "hot-enrich", solutePartitionWeight: .24,
  ...
  dendritic: {
    ...
    settings: { confinement: "sphere", geometryPreference: "strain", geometricStrainWeight: .08,
      compositionPreference: "soft", chargePreference: "auto", surfacePreference: "soft", growthDrivingMode: "volume-driven", growthDrivingWeight: .48, attachmentTopologyMode: "kink", attachmentTopologyWeight: .48, habitAnisotropyMode: "roughening", habitAnisotropyWeight: .48, defectPrecursorMode: "explore", coherencyMemoryMode: "isolate",
  ...
  impingement: {
    ...
    settings: { confinement: "box", geometryPreference: "strain", geometricStrainWeight: .16,
      compositionPreference: "soft", chargePreference: "auto", surfacePreference: "soft", growthDrivingMode: "balanced", attachmentTopologyMode: "kink", habitAnisotropyMode: "roughening", defectPrecursorMode: "seam", coherencyMemoryMode: "relieve",
  ...
  "pore-fill": {
    ...
    settings: { confinement: "hourglass", geometryPreference: "strain", geometricStrainWeight: .16,
      compositionPreference: "soft", chargePreference: "auto", surfacePreference: "strong", growthDrivingMode: "surface-limited", attachmentTopologyMode: "kink", defectPrecursorMode: "suppress", coherencyMemoryMode: "isolate",
  ...
  const receipt = {
    ...
    search: searchVisible ? {
      ...
      solutePartitionRanking: {
        ...
        mode: solutePartitionMode,
        label: solutePartitionLabel(),
        enabled: activeSolutePartitionWeight() > 0,
        effectiveWeight: activeSolutePartitionWeight(),
        selectedSpecies: resolvedSoluteSpecies(),
        speciesSelectionPolicy: "user-selected from observed species; least-abundant observed species is the initial default",
        observedReferenceFraction: receiptRound(soluteSpeciesVocabulary()
          .find((entry) => entry.species === resolvedSoluteSpecies())?.fraction || 0),
        acceptedMeanScore: receiptRound(acceptedSolutePartitionScore / Math.max(1, acceptedDecisions)),
        rejectedMeanScore: receiptRound(rejectedSolutePartitionScore / Math.max(1, rejectedDecisions)),
        evaluations: solutePartitionEvaluations,
      ...
      actionDefectPrecursorRanking: {
        role: "target-blind soft ordering of unchanged exact actions by action-generated geometric precursor burden",
        mode: defectPrecursorMode,
        label: defectPrecursorLabel(),
        enabled: activeDefectPrecursorWeight() > 0,
        effectiveWeight: activeDefectPrecursorWeight(),
        componentWeights: { exposedCoordination: .35, strainOutlier: .30,
          compositionDrift: .15, incompatibleSeam: .20 },
        acceptedMeanScore: receiptRound(acceptedDefectPrecursorScore / Math.max(1, acceptedDecisions)),
        rejectedMeanScore: receiptRound(rejectedDefectPrecursorScore / Math.max(1, rejectedDecisions)),
        acceptedMeanBurden: receiptRound(acceptedDefectPrecursorBurden / Math.max(1, acceptedDecisions)),
        rejectedMeanBurden: receiptRound(rejectedDefectPrecursorBurden / Math.max(1, rejectedDecisions)),
        evaluations: defectPrecursorEvaluations,
        preexistingMicrostructureRolesUsed: false,
        candidateSetChanged: false,
        candidateGeometryChanged: false,
        hardAdmissionChanged: false,
        heldoutTargetUsed: false,
        defectIdentityAssigned: false,
        formationEnergyInferred: false,
      ...
      coherencyMemoryRanking: {
        role: "target-blind finite-range cluster-graph memory of mismatch carried only by accepted placements",
        mode: coherencyMemoryMode,
        label: coherencyMemoryLabel(),
        enabled: activeCoherencyMemoryWeight() > 0,
        reachInClusterGraphHops: coherencyMemoryReach,
        effectiveWeight: activeCoherencyMemoryWeight(),
        hopWeight: "1/(1+hop)",
        acceptedMeanScore: receiptRound(acceptedCoherencyMemoryScore / Math.max(1, acceptedDecisions)),
        rejectedMeanScore: receiptRound(rejectedCoherencyMemoryScore / Math.max(1, rejectedDecisions)),
        acceptedMeanInheritedMismatch: receiptRound(acceptedInheritedMismatch / Math.max(1, acceptedDecisions)),
        rejectedMeanInheritedMismatch: receiptRound(rejectedInheritedMismatch / Math.max(1, rejectedDecisions)),
        evaluations: coherencyMemoryEvaluations,
...
function notebookInterventionFactors(receipt) {
  ...
  return {
    ...
    softPhysics: { label: "soft physics ordering", role: "search", value: serialized(search ? {
      ...
      actionDefectPrecursors: [search.actionDefectPrecursorRanking?.mode,
        search.actionDefectPrecursorRanking?.effectiveWeight],
...
function defectPrecursorLabel(mode = defectPrecursorMode) {
  return ({ none: "action burden diagnostic", suppress: "geometric-precursor suppression",
    explore: "defect-rich growth exploration", seam: "seam-accommodation hypothesis" })[mode]
    || "action burden diagnostic";
}

function activeDefectPrecursorWeight() {
  return defectPrecursorMode === "none" ? 0 : defectPrecursorWeight;
}

function defectPrecursorsForAction({ geometricStrain, surfaceCompletion, compositionBalance, loopClosure },
  { recordWork = true } = {}) {
  const components = {
    exposedCoordination: Math.tanh(Math.max(0, surfaceCompletion.newSiteDeficit)),
    strainOutlier: Math.tanh(Math.max(0, geometricStrain.total)),
    compositionDrift: Math.tanh(2 * Math.max(0, compositionBalance.scaledDelta)),
    incompatibleSeam: Math.tanh(Math.max(0, loopClosure.independentConflictingPaths) / 2),
  };
  const burden = .35 * components.exposedCoordination + .30 * components.strainOutlier
    + .15 * components.compositionDrift + .20 * components.incompatibleSeam;
  const score = defectPrecursorMode === "suppress" ? 1 - 2 * burden
    : defectPrecursorMode === "explore" ? 2 * burden - 1
      : defectPrecursorMode === "seam" ? THREE.MathUtils.clamp(
        2 * components.incompatibleSeam + .5 * components.strainOutlier
          - .5 * components.exposedCoordination - 1, -1, 1) : 0;
  const dominantPrecursor = Object.entries(components)
    .sort((first, second) => second[1] - first[1] || first[0].localeCompare(second[0]))[0][0];
  if (recordWork) defectPrecursorEvaluations++;
  return { mode: defectPrecursorMode, label: defectPrecursorLabel(), enabled: defectPrecursorMode !== "none",
    score, burden, components, dominantPrecursor,
    weights: { exposedCoordination: .35, strainOutlier: .30, compositionDrift: .15, incompatibleSeam: .20 },
...
function coherencyMemoryLabel(mode = coherencyMemoryMode) {
  return ({ none: "local-history diagnostic", continue: "coherent-domain continuation",
    relieve: "local mismatch relief", isolate: "strain-hotspot avoidance" })[mode]
    || "local-history diagnostic";
}

function activeCoherencyMemoryWeight() {
  return coherencyMemoryMode === "none" ? 0 : coherencyMemoryWeight;
}

function coherencyMemoryForCandidate(candidate, evaluation, { recordWork = true } = {}) {
  const parent = placedClusters.find((placement) => placement.id === candidate.parentId);
  const startIds = new Set([candidate.parentId]);
  evaluation.merged.forEach(({ atom }) => (atom.clusterIds || []).forEach((id) => startIds.add(id)));
  const visited = new Map([...startIds].filter(Number.isInteger).map((id) => [id, 0]));
  let frontier = [...visited.keys()];
  for (let hop = 0; hop < coherencyMemoryReach && frontier.length; hop++) {
    const next = new Set();
    frontier.forEach((id) => {
      const placement = placedClusters.find((entry) => entry.id === id);
      [placement?.parentId, ...placedClusters.filter((entry) => entry.parentId === id).map((entry) => entry.id)]
        .filter(Number.isInteger).forEach((neighborId) => next.add(neighborId));
      (placement?.atomIds || []).forEach((atomId) => {
        const atom = atoms.find((entry) => entry.id === atomId);
        (atom?.clusterIds || []).forEach((neighborId) => next.add(neighborId));
      });
    });
    frontier = [...next].filter((id) => !visited.has(id));
    frontier.forEach((id) => visited.set(id, hop + 1));
  }
  const memories = [...visited.entries()].map(([id, hop]) => {
    const memory = placedClusters.find((entry) => entry.id === id)?.coherencyMemory;
    return memory ? { ...memory, hop, weight: 1 / (1 + hop) } : null;
  }).filter(Boolean);
  ...
  const totalWeight = memories.reduce((sum, memory) => sum + memory.weight, 0);
  const inheritedMismatch = totalWeight ? memories.reduce((sum, memory) => sum
    + memory.mismatch * memory.weight, 0) / totalWeight : 0;
  const orientationAgreement = totalWeight ? memories.reduce((sum, memory) => sum
    + Math.abs(candidateAxis.dot(new THREE.Vector3(...memory.axis))) * memory.weight, 0) / totalWeight : 0;
  const mismatchGradient = Math.abs(candidateMismatch - inheritedMismatch);
  const hotspot = memories.length ? Math.max(...memories.map((memory) => memory.mismatch)) : 0;
  const score = !memories.length ? 0
    : coherencyMemoryMode === "continue" ? THREE.MathUtils.clamp(orientationAgreement - mismatchGradient, -1, 1)
      : coherencyMemoryMode === "relieve" ? THREE.MathUtils.clamp(2 * (inheritedMismatch - candidateMismatch), -1, 1)
        : coherencyMemoryMode === "isolate" ? THREE.MathUtils.clamp(1 - inheritedMismatch - candidateMismatch, -1, 1) : 0;
  if (recordWork) coherencyMemoryEvaluations++;
  return { mode: coherencyMemoryMode, label: coherencyMemoryLabel(), enabled: coherencyMemoryMode !== "none",
    reach: coherencyMemoryReach, score, support: memories.length, candidateMismatch, inheritedMismatch,
    mismatchGradient, orientationAgreement, hotspot, candidateAxis: candidateAxis.toArray(),
    graphHopWeights: "1/(1+hop)", acceptedHistoryOnly: true, actionGeometryChanged: false,
...
function capturePolicyComparison(entries) {
  ...
  const policies = [
    ...
    { id: "defect-precursors", label: `${defectPrecursorLabel()} ${activeDefectPrecursorWeight().toFixed(2)}`,
...
function soluteSpeciesVocabulary() {
  ...
  referenceAtoms.forEach((atom) => {
    if (occupationalAlternatives(atom.species)) return;
    counts.set(atom.species, (counts.get(atom.species) || 0) + 1);
  });
  return [...counts.entries()].sort((first, second) => first[1] - second[1]
    || String(first[0]).localeCompare(String(second[0])))
    .map(([species, count]) => ({ species, count, fraction: count / Math.max(1, referenceAtoms.length) }));
}

function resolvedSoluteSpecies() {
  const vocabulary = soluteSpeciesVocabulary();
  if (vocabulary.some((entry) => entry.species === soluteSpecies)) return soluteSpecies;
  return vocabulary.length >= 2 ? vocabulary[0].species : null;
}

function renderSoluteSpeciesOptions() {
  const vocabulary = soluteSpeciesVocabulary();
  const signature = vocabulary.map((entry) => `${entry.species}:${entry.count}`).join("|");
  if (soluteSpeciesSelect.dataset.signature !== signature) {
    soluteSpeciesSelect.replaceChildren();
    if (vocabulary.length < 2) {
      const option = document.createElement("option"); option.value = "";
      option.textContent = vocabulary.length ? `${vocabulary[0].species} · single-component` : "No species available";
      soluteSpeciesSelect.append(option);
    } else vocabulary.forEach((entry) => {
      const option = document.createElement("option"); option.value = entry.species;
      option.textContent = `${entry.species} · ${(entry.fraction * 100).toFixed(1)}% observed`;
      soluteSpeciesSelect.append(option);
    });
    soluteSpeciesSelect.dataset.signature = signature;
  }
  soluteSpecies = resolvedSoluteSpecies();
  soluteSpeciesSelect.value = soluteSpecies || "";
  return vocabulary;
}

function activeSolutePartitionWeight() {
  return solutePartitionMode === "none" || !resolvedSoluteSpecies() ? 0 : solutePartitionWeight;
}

function solutePartitionLabel(mode = solutePartitionMode) {
  return ({ none: "homogeneous composition", "cold-enrich": "cold-side enrichment",
    "hot-enrich": "hot-side rejection", "surface-enrich": "open-interface segregation",
    "bulk-enrich": "occupied-pocket incorporation" })[mode] || "homogeneous composition";
}

function solutePartitionForFreshSites(fresh, thermalField, capillaryGeometry, { recordWork = true } = {}) {
  const species = resolvedSoluteSpecies(); const vocabulary = soluteSpeciesVocabulary();
  const target = vocabulary.find((entry) => entry.species === species);
  const referenceFraction = target?.fraction || 0;
  const emittedCount = species ? fresh.filter((site) => site.species === species).length : 0;
  const emittedFraction = emittedCount / Math.max(1, fresh.length);
  const contrastScale = Math.max(.15, Math.sqrt(Math.max(0, referenceFraction * (1 - referenceFraction))));
  const enrichmentContrast = species && fresh.length
    ? Math.tanh((emittedFraction - referenceFraction) / contrastScale) : 0;
  let spatialScore = 0; let fieldSource = "none"; let available = vocabulary.length >= 2;
  if (solutePartitionMode === "cold-enrich" || solutePartitionMode === "hot-enrich") {
    available = available && Boolean(thermalField?.enabled);
    spatialScore = thermalField?.enabled ? thermalField.score * (solutePartitionMode === "cold-enrich" ? 1 : -1) : 0;
    fieldSource = thermalField?.enabled ? "declared reduced thermal field" : "thermal field required";
  } else if (solutePartitionMode === "surface-enrich") {
    spatialScore = capillaryGeometry?.exposedScore || 0; fieldSource = "open solid-angle fraction";
  } else if (solutePartitionMode === "bulk-enrich") {
    spatialScore = capillaryGeometry?.pocketScore || 0; fieldSource = "occupied solid-angle fraction";
  }
  const score = solutePartitionMode !== "none" && available ? enrichmentContrast * spatialScore : 0;
  if (recordWork && solutePartitionMode !== "none" && available) solutePartitionEvaluations++;
  return {
    mode: solutePartitionMode, label: solutePartitionLabel(), enabled: solutePartitionMode !== "none" && available,
    species, observedReferenceFraction: referenceFraction, emittedCount, emittedSites: fresh.length,
    emittedFraction, enrichmentContrast, spatialScore, fieldSource, score,
    speciesChosenFromObservedConfiguration: true, candidateSetChanged: false,
    candidateGeometryChanged: false, hardAdmissionChanged: false, heldoutTargetUsed: false,
    chemicalPotentialInferred: false, partitionCoefficientInferred: false,
    phaseDiagramUsed: false, interfacialEnergyInferred: false, diffusionIntegrated: false,
    physicalTimeIntegrated: false,
  };
}
...
function initializeOffLatticeSearch() {
  ...
  acceptedDefectPrecursorScore = 0;
  rejectedDefectPrecursorScore = 0;
  acceptedDefectPrecursorBurden = 0;
  rejectedDefectPrecursorBurden = 0;
  defectPrecursorEvaluations = 0;
  acceptedCoherencyMemoryScore = 0;
  rejectedCoherencyMemoryScore = 0;
...
function syncStageOptions() {
  ...
  if (training) {
    ...
    defectPrecursorSelect.value = defectPrecursorMode;
    defectPrecursorWeightSelect.value = String(defectPrecursorWeight);
    ...
    defectPrecursorSelect.disabled = finiteIceAnchorMode;
    defectPrecursorWeightSelect.disabled = finiteIceAnchorMode || defectPrecursorMode === "none";
    ...
    defectPrecursorHint.textContent = defectPrecursorMode === "none"
    ...
    soluteSpeciesHint.textContent = selectedSolute
      ...
      : "choose a multicomponent sample";
    solutePartitionHint.textContent = solutePartitionMode === "none"
      ? "homogeneous · weight zero"
      : (solutePartitionMode === "cold-enrich" || solutePartitionMode === "hot-enrich") && thermalFieldMode === "none"
        ? "requires reduced thermal field · score zero"
        : `${solutePartitionLabel()} · weight ${solutePartitionWeight.toFixed(2)}`;
    ...
    const defectPrecursorUse = defectPrecursorMode === "none"
      ? " Action-generated geometric defect precursors are reported but contribute zero ranking weight."
      : ` A ${defectPrecursorWeight.toFixed(2)} soft ${defectPrecursorLabel()} term combines exposed coordination, local strain, composition drift, and incompatible multi-parent seams without assigning a defect identity.`;
...
function performOffLatticeEvent() {
  ...
  batch.forEach(({ candidate, evaluation: snapshotEvaluation }) => {
    ...
    if (!snapshotEvaluation.accepted) {
      ...
      rejectedDefectPrecursorScore += snapshotEvaluation.defectPrecursors.score;
    ...
    acceptedDefectPrecursorScore += evaluation.defectPrecursors.score;
...
function rebuildWorld() {
  ...
  currentCandidates.forEach((candidate, candidateIndex) => {
    ...
    if (candidateIndex < 12 && candidate.defectPrecursors) {
      ...
      const precursor = candidate.defectPrecursors;
      const halo = new THREE.Mesh(new THREE.DodecahedronGeometry(.34 + .28 * precursor.burden, 0),
        new THREE.MeshBasicMaterial({ color: precursorColors[precursor.dominantPrecursor] || 0xff7f88,
          wireframe: true, transparent: true,
          opacity: defectPrecursorMode === "none" ? .14 : .28 + .46 * precursor.burden, depthWrite: false }));
      halo.position.copy(candidate.p); decisionGroup.add(halo);
    }
    if (candidateIndex < 12 && candidate.coherencyMemory?.support > 0) {
      const memory = candidate.coherencyMemory;
      const axis = new THREE.Vector3(...memory.candidateAxis).normalize();
      const memoryColor = memory.score >= 0 ? 0x55c8ff : 0xff7f88;
      const span = .22 + .55 * memory.inheritedMismatch;
      const line = new THREE.Line(
        new THREE.BufferGeometry().setFromPoints([
          candidate.p.clone().addScaledVector(axis, -span),
          candidate.p.clone().addScaledVector(axis, span),
        ]),
        new THREE.LineBasicMaterial({ color: memoryColor, transparent: true,
          opacity: coherencyMemoryMode === "none" ? .18 : .72 }),
      );
      decisionGroup.add(line);
      const memoryGlyph = new THREE.Mesh(new THREE.OctahedronGeometry(.20 + .18 * memory.hotspot, 0),
        new THREE.MeshBasicMaterial({ color: memoryColor, wireframe: true, transparent: true,
          opacity: .24 + .38 * Math.min(1, memory.support / 4), depthWrite: false }));
      memoryGlyph.position.copy(candidate.p);
      memoryGlyph.scale.set(1 + memory.orientationAgreement, .72, .72);
  ...
  return [
    ...
    { id: "solute-partition", process: "solute partitioning / interfacial segregation", status: activeSolutePartitionWeight() > 0 ? "soft" : "open", role: activeSolutePartitionWeight() > 0 ? "species × spatial-field ordering" : "disabled",
      encoding: activeSolutePartitionWeight() > 0
        ...
        : "no species-resolved spatial partition term",
      evidence: leap ? `Accepted mean partition score ${receiptRound(acceptedSolutePartitionScore / Math.max(1, acceptedDecisions), 4)}; rejected mean ${receiptRound(rejectedSolutePartitionScore / Math.max(1, rejectedDecisions), 4)}.` : "No partitioned attachment evaluated yet.",
      boundary: "This is a counterfactual spatial redistribution hypothesis, not a chemical potential, equilibrium partition coefficient, phase diagram, solute diffusion, interfacial free energy, constitutional-supercooling calculation, or physical time." },
    ...
    { id: "defect-precursors", process: "action-generated defect-precursor geometry", status: activeDefectPrecursorWeight() > 0 ? "soft" : "open", role: activeDefectPrecursorWeight() > 0 ? "bounded multi-channel action ordering" : "diagnostic",
      encoding: `${defectPrecursorLabel()}; 35% exposed coordination + 30% local strain + 15% composition drift + 20% incompatible multi-parent seam`,
      evidence: leap ? `Accepted mean precursor burden ${receiptRound(acceptedDefectPrecursorBurden / Math.max(1, acceptedDecisions), 4)}; rejected mean ${receiptRound(rejectedDefectPrecursorBurden / Math.max(1, rejectedDecisions), 4)}; ${defectPrecursorEvaluations.toLocaleString()} actions audited.` : "No action-generated precursor evaluated yet.",
      boundary: "This budget assigns no vacancy, antisite, stacking fault, or dislocation. It infers no formation energy, migration barrier, mobility, rate, or physical time." },
    { id: "coherency-memory", process: "history-dependent coherency / local mismatch propagation", status: activeCoherencyMemoryWeight() > 0 ? "soft" : "open", role: activeCoherencyMemoryWeight() > 0 ? "accepted-history cluster-graph ordering" : "diagnostic",
      encoding: `${coherencyMemoryLabel()}; R${coherencyMemoryReach} cluster-graph hops weighted 1/(1+hop)`,
      evidence: leap ? `Accepted mean inherited mismatch ${receiptRound(acceptedInheritedMismatch / Math.max(1, acceptedDecisions), 4)}; rejected mean ${receiptRound(rejectedInheritedMismatch / Math.max(1, rejectedDecisions), 4)}; ${coherencyMemoryEvaluations.toLocaleString()} candidates audited.` : "No accepted-history coherency neighborhood evaluated yet.",
      boundary: "This transports a bounded sample-relative geometric mismatch mark. It is not stress, strain energy, elastic modulus, force balance, relaxation, plasticity, a dislocation calculation, or physical time." },
  ...
  const evidence = {
    ...
    "solute partition": {
      observed: state?.solutePartition?.species
        ...
        : "no multicomponent partition species available",
      encoding: state?.solutePartition?.enabled
        ? `bounded enrichment contrast × ${state.solutePartition.fieldSource} score`
        : solutePartitionMode === "none" ? "homogeneous composition control" : "required spatial field unavailable",
      searchRole: activeSolutePartitionWeight() > 0
        ? `Soft ${solutePartitionLabel()} rank term with weight ${activeSolutePartitionWeight().toFixed(2)}.` : "Disabled; weight zero.",
      boundary: "This is not a chemical potential, equilibrium partition coefficient, phase diagram, solute diffusion, interfacial energy, or constitutional-supercooling calculation.",
    },
    ...
    "defect precursors": {
      observed: state?.defectPrecursors
        ? `burden ${state.defectPrecursors.burden.toFixed(3)} · dominant ${state.defectPrecursors.dominantPrecursor}`
        : "no action-generated precursor evaluated",
      encoding: state?.defectPrecursors
        ? `coord ${state.defectPrecursors.components.exposedCoordination.toFixed(3)} · strain ${state.defectPrecursors.components.strainOutlier.toFixed(3)} · composition ${state.defectPrecursors.components.compositionDrift.toFixed(3)} · seam ${state.defectPrecursors.components.incompatibleSeam.toFixed(3)}`
        : "bounded four-channel action burden",
      searchRole: activeDefectPrecursorWeight() > 0
        ? `Soft ${defectPrecursorLabel()} rank term with weight ${activeDefectPrecursorWeight().toFixed(2)}.` : "Diagnostic only; weight zero.",
      boundary: "A precursor burden is not a vacancy, antisite, stacking-fault, or dislocation assignment and carries no formation energy, barrier, mobility, rate, or time.",
    },
    "coherency memory": {
      observed: state?.coherencyMemory?.support
        ? `${state.coherencyMemory.support} accepted placement memories within R${state.coherencyMemory.reach} · inherited mismatch ${state.coherencyMemory.inheritedMismatch.toFixed(3)}`
        : "no accepted mismatch memory in the selected graph neighborhood",
      encoding: state?.coherencyMemory
        ? `candidate ${state.coherencyMemory.candidateMismatch.toFixed(3)} · gradient ${state.coherencyMemory.mismatchGradient.toFixed(3)} · axis agreement ${state.coherencyMemory.orientationAgreement.toFixed(3)}`
        : "accepted-placement mismatch marks on the cluster graph",
      searchRole: activeCoherencyMemoryWeight() > 0
...
function renderConstraintLedger(state, mode = "configured") {
  ...
  const terms = state ? [
    ...
    { name: "defect precursors", status: ranked(activeDefectPrecursorWeight() > 0),
      value: state.defectPrecursors ? `${signed(state.defectPrecursors.score)} · burden ${state.defectPrecursors.burden.toFixed(3)} · ${state.defectPrecursors.dominantPrecursor}` : "not evaluated",
      detail: activeDefectPrecursorWeight() > 0 ? `${defectPrecursorLabel()} · rank weight ${activeDefectPrecursorWeight().toFixed(2)}` : "diagnostic · identities withheld" },
    ...
    { name: "defect precursors", status: "diagnostic", value: "withheld", detail: "no executable action" },
    { name: "coherency memory", status: "diagnostic", value: "withheld", detail: "no accepted growth history" },
    ...
    { name: "defect precursors", status: "diagnostic", value: "not used", detail: "specialized frozen trace" },
    { name: "coherency memory", status: "diagnostic", value: "not used", detail: "specialized frozen trace" },
...
soluteSpeciesSelect.addEventListener("change", () => {
  soluteSpecies = soluteSpeciesSelect.value || null;
  if (pipelineStage === 4) enterPipelineStage(4);
  else syncStageOptions();
});
solutePartitionSelect.addEventListener("change", () => {
  const value = solutePartitionSelect.value;
  solutePartitionMode = ["cold-enrich", "hot-enrich", "surface-enrich", "bulk-enrich"].includes(value)
    ? value : "none";
  if (pipelineStage === 4) enterPipelineStage(4);
  else syncStageOptions();
});
solutePartitionWeightSelect.addEventListener("change", () => {
  const value = Number(solutePartitionWeightSelect.value);
  solutePartitionWeight = [.12, .24, .48].includes(value) ? value : .24;
  if (pipelineStage === 4) enterPipelineStage(4);
  else syncStageOptions();
});
...
defectPrecursorSelect.addEventListener("change", () => {
  const value = defectPrecursorSelect.value;
  defectPrecursorMode = ["suppress", "explore", "seam"].includes(value) ? value : "none";
  if (pipelineStage === 4) enterPipelineStage(4);
  else syncStageOptions();
});
defectPrecursorWeightSelect.addEventListener("change", () => {
  const value = Number(defectPrecursorWeightSelect.value);
  defectPrecursorWeight = [.12, .24, .48].includes(value) ? value : .24;
  if (pipelineStage === 4) enterPipelineStage(4);
  else syncStageOptions();
});
coherencyMemorySelect.addEventListener("change", () => {
  const value = coherencyMemorySelect.value;
  coherencyMemoryMode = ["continue", "relieve", "isolate"].includes(value) ? value : "none";
  if (pipelineStage === 4) enterPipelineStage(4);
  else syncStageOptions();
});
coherencyReachSelect.addEventListener("change", () => {
  const value = Number(coherencyReachSelect.value);
  coherencyMemoryReach = [1, 2, 3].includes(value) ? value : 2;
  if (pipelineStage === 4) enterPipelineStage(4);
  else syncStageOptions();
});
