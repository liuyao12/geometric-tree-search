...
function updateRecursiveBenchmark() {
  ...
  if (benchmark.connection) {
    ...
    if (benchmark.connection.macro) {
      ...
      verdict.innerHTML = `<span>${macro.safe}</span><strong>${macro.rejected}</strong>${macro.crystal ? `<small>${macro.crystal}</small>` : ""}`;
