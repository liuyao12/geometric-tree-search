#!/usr/bin/env python3
"""One-shot fresh full-width IQC confirmation with an auditable raw receipt."""
...
from materials_gcts_iqc_parent_balanced_confirmation_preregistration_v3 import (
    ...
    RUNTIME_LIMIT_SECONDS, SECOND_RADIUS, SEED_RADIUS, THIRD_RADIUS,
...
def _labels(actions, truth):
    return tuple(colored_action_labels(
...
def _execute_and_score_once():
    ...
    if (execution.target_used or execution.parent_width != PARENT_WIDTH or
        ...
        raise AssertionError("V3 fresh receipt invariant drift")
    ...
    raw_exact = tuple(index for index, row in enumerate(raw_labels)
                      if all(row))
    selected_set = frozenset(execution.selected_nine_action_lineage_indices)
    ...
    terminal_exact = tuple(index for index, row in enumerate(terminal_labels)
                           if all(row))
    raw_exact_parents = tuple(sorted({
        execution.raw_nine_action_lineages[index].parent_id
        for index in raw_exact}))
    selected_exact_parents = tuple(sorted({
        execution.raw_nine_action_lineages[index].parent_id
        for index in selected_exact}))
    complete_exact_parents = tuple(sorted({
    ...
    every_raw_exact_parent_retained = (
        set(raw_exact_parents) <= set(selected_exact_parents))
    success = bool(
        raw_exact and selected_exact and complete_exact and
        every_raw_exact_parent_retained and receipt_unchanged and
    ...
    body = {
        ...
        "raw_exact_parent_classes": raw_exact_parents,
        ...
        "selected_exact_parent_classes": selected_exact_parents,
        "every_raw_exact_parent_represented_after_selection":
            every_raw_exact_parent_retained,
        ...
        "exact_complete_parent_lineages": complete_exact_parents,
...
def validate_result(row):
    ...
    if (hashlib.sha256(canonical_json(body)).hexdigest() != digest or
        ...
        raise AssertionError("V3 fresh confirmation drift")
    if EXPECTED_RESULT_DIGEST and digest != EXPECTED_RESULT_DIGEST:
        raise AssertionError("V3 fresh result digest drift")
    return row
...
def load_default_result(path=DEFAULT_FIXTURE):
    ...
    if (EXPECTED_FIXTURE_SHA256 and
        ...
        raise AssertionError("V3 fresh fixture byte drift")
...
def _summary(row):
    return {key: row[key] for key in (
        ...
        "raw_exact_nine_action_lineages", "selected_nine_action_lineages",
        "selected_exact_nine_action_lineages", "fourth_candidates",
        "exact_complete_twelve_action_paths", "exact_terminal_fourth_blocks",
        "best_complete_correct_actions", "every_raw_exact_parent_represented_after_selection",
        "total_execution_seconds", "runtime_gate_passed", "receipt_digest",
        "result_digest")}
...
def main():
    ...
    if args.write:
        ...
        if DEFAULT_FIXTURE.exists() or ATTEMPT_MARKER.exists():
            raise RuntimeError("V3 fresh one-shot already consumed")
