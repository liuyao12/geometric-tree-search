...
(async()=>{
 ...
 try{
  ...
  await page.goto(base+'/3d-lattice-tiler/?experiment=window&catalogue=all');await page.waitForFunction(()=>document.querySelector('#status').textContent.startsWith('Ready'));assert.equal(await page.locator('#tile option').count(),54);assert.ok(await page.locator('#reference').isVisible());
  ...
  await page.goto(base+'/apps/3d-lattice-tiler/index.html?experiment=window');await page.waitForFunction(()=>document.querySelector('#status').textContent.startsWith('Ready'));assert.equal(await page.locator('#tile option').count(),40);assert.equal(await page.locator('#radius').inputValue(),'18');
