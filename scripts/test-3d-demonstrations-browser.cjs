...
(async()=>{
 ...
 try{
  ...
  await page.click('#catalogueLink');await page.waitForFunction(()=>document.querySelector('#status').textContent.startsWith('Ready'));assert.equal(await page.locator('#tile option').count(),54);assert.ok(await page.locator('#reference').isVisible());
  ...
  await page.goto(base+'/apps/3d-lattice-tiler/index.html');await page.waitForFunction(()=>document.querySelector('#status').textContent.startsWith('Ready'));assert.equal(await page.locator('#tile option').count(),1);assert.equal(await page.locator('#radius').inputValue(),'18');
