...
import { A2_SLICED_SIZE10_CANDIDATES } from "../assets/a2-sliced-size10-candidates.js";
import {
...
assert.equal(candidates.length, 56 + A2_LAYERED_SIZE7_CANDIDATES.length + A2_LAYERED_SIZE8_CANDIDATES.length + A2_LAYERED_SIZE9_CANDIDATES.length + A2_SLICED_SIZE7_CANDIDATES.length + A2_SLICED_SIZE8_CANDIDATES.length + A2_SLICED_SIZE9_CANDIDATES.length + A2_SLICED_SIZE9_PALINDROMIC_CANDIDATES.length + A2_SLICED_SIZE10_CANDIDATES.length,
...
const unresolvedA2Candidates = [...A2_SLICED_SIZE10_CANDIDATES, ...A2_SLICED_SIZE9_CANDIDATES, ...A2_SLICED_SIZE9_PALINDROMIC_CANDIDATES, ...A2_SLICED_SIZE8_CANDIDATES, ...A2_SLICED_SIZE7_CANDIDATES, ...A2_LAYERED_SIZE9_CANDIDATES, ...A2_LAYERED_SIZE8_CANDIDATES, ...A2_LAYERED_SIZE7_CANDIDATES].filter(
...
const palindromicUnresolved = A2_SLICED_SIZE9_PALINDROMIC_CANDIDATES.filter(
  candidate => candidate.screening.status === "inconclusive"
);
...
assert.ok(palindromicUnresolved.every(candidate =>
  ...
  && candidate.screening.periodic_eight_copy_solver_unknowns === 0
  ...
  && candidate.screening.radius_two_status === "unresolved"
  && candidate.screening.radius_two_rounds === 256
  && candidate.screening.radius_two_failure_clauses === 256
...
const fiveCopyLeader = palindromicUnresolved.find(candidate => candidate.id === "a2sp_9_04636");
assert.deepEqual(fiveCopyLeader?.screening.five_copy_substitution_exact_scales, [2]);
assert.deepEqual(fiveCopyLeader?.screening.five_copy_substitution_models, ["proper"]);
assert.equal(fiveCopyLeader?.screening.five_copy_substitution_certified_negatives, 1);
assert.equal(fiveCopyLeader?.screening.five_copy_substitution_parents_exhausted, 17707);
assert.ok(palindromicUnresolved.filter(candidate => candidate.id !== "a2sp_9_04636")
  .every(candidate => candidate.screening.five_copy_substitution_certified_negatives === 0));
const palindromicPeriodic = A2_SLICED_SIZE9_PALINDROMIC_CANDIDATES.filter(
  candidate => candidate.screening.status === "periodic"
);
assert.deepEqual(palindromicPeriodic.map(candidate => candidate.id), [
  "a2sp_9_15353", "a2sp_9_17745"
]);
assert.ok(palindromicPeriodic.every(candidate =>
  candidate.screening.certificate === "translational"
  && candidate.screening.motif_tiles === 8
  && candidate.screening.quotient_determinant === 12
  && candidate.screening.periodic_eight_copy_replay_verified === true
  && candidate.screening.periodic_template?.motif?.length === 8
  && candidate.screening.periodic_quotient_cluster_substitution?.certified === true
  && candidate.screening.periodic_quotient_cluster_substitution?.child_metatile_copies === 8
  && candidate.screening.periodic_quotient_cluster_substitution?.expanded_tile_copies === 64
  && candidate.screening.periodic_quotient_cluster_substitution?.replay?.verified === true
));
...
assert.ok(A2_SLICED_SIZE10_CANDIDATES.filter(candidate => candidate.screening.status === "inconclusive").every(candidate =>
  ...
  && candidate.screening.four_copy_substitution_models.join(",") === "proper,reflected"
  && candidate.screening.four_copy_substitution_certified_negatives === 2
  && candidate.screening.four_copy_substitution_parents_exhausted > 446000
));
assert.deepEqual(
