...
def test_metric_ports_execute_exactly_then_honestly_stall() -> None:
    ...
    assert result.final_atoms == 17979
    assert result.waves[0].accepted_sites == 860
    assert result.waves[0].true_accepted_sites == 860
    assert result.waves[0].precision == 1.0
    assert result.waves[1].one_vote_candidates == 13020
    assert result.waves[1].accepted_sites == 13020
    assert result.waves[1].true_accepted_sites == 0
    assert result.waves[1].precision == 0.0
