#!/usr/bin/env python3
"""Third fresh IQC preregistration for the auditable full-width executor."""
...
from materials_gcts_iqc_parent_balanced_confirmation_preregistration import (
    ...
    SECOND_RADIUS, SEED_RADIUS, SPATIAL_GUARD, THIRD_RADIUS, canonical_json)
from materials_gcts_iqc_parent_balanced_confirmation_preregistration_v2 \
    import CONFIRMATION_CENTER as V2_CENTER, FAILED_V1_CENTER


PRIOR_CENTERS = tuple(sorted(set(
    V1_PRIORS + (FAILED_V1_CENTER, V2_CENTER))))
CONFIRMATION_CENTER = (-180., -80., -300.)
EXECUTION_WORKERS = 4
...
def select_confirmation_center():
    ...
    if not rows:
        raise AssertionError("empty V3 parent-balanced center grid")
    return max(rows)[-1]


MANIFEST = {
    "schema_version": 3,
    "protocol_kind": "fresh auditable full-width parent-balanced IQC",
    ...
    "target_order": (
        "verify protocol, source hashes, geometry and frozen policies",
        "open only the fresh radius-9 seed",
        "freeze and serialize every raw nine-action lineage",
        "select 64 parents and freeze all 512 twelve-action candidates",
        "open the radius-41.562 target exactly once",
        "score raw, selected and complete lineages without refit or retry",
    ),
    "success_gate": {
        ...
        "every_raw_exact_parent_represented_after_selection": True,
...
EXPECTED_MANIFEST_DIGEST = \
    "e5b8a7d33f394b4750d0b3fdd6298eb616d3bf1096f17d903fe1af69a3b49f84"
...
def validate_preregistration(*, verify_sources=True):
    ...
    if (select_confirmation_center() != CONFIRMATION_CENTER or
        ...
        raise AssertionError("V3 parent-balanced preregistration drift")
    if verify_sources and source_file_digests() != SOURCE_FILE_SHA256:
        raise AssertionError("V3 parent-balanced source hash drift")
    return digest
...
class OneShotOrderGuard:
    ...
    def receipt_frozen(self, digest):
        if (self.state != "seed-opened" or not isinstance(digest, str) or
                len(digest) != 64):
            raise RuntimeError("V3 receipt order violation")
        int(digest, 16)
    ...
    def target_opened(self):
        if self.state != "receipt-frozen" or self.target_open_count:
            raise RuntimeError("V3 target order violation")
    ...
    def scored(self, digest):
        if (self.state != "target-opened" or self.score_count or
            ...
            raise RuntimeError("V3 score order violation")
        self.score_count += 1
        self.state = "scored"
