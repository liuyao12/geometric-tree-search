...
result={'date':'2026-09-24','scope':'Potential non-tilers only. Known space-fillers and periodic controls excluded; aliases consolidated.','newlyCertifiedPeriodic':[read(p) for p in sorted((F/'periodic').glob('*.json')) if read(p).get('verified')],'rows':rows,'benchmarks':benchmarks,'counts':{'candidates':len(rows),'finite':sum(r['exact'] is not None for r in rows),'zero':sum(r['exact']==0 for r in rows),'one':sum(r['exact']==1 for r in rows),'unresolved':sum(r['exact'] is None for r in rows)}}
