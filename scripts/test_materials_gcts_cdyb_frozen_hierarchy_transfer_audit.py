...
def test_cdyb_frozen_hierarchy_transfer_is_exact_reencoding_only():
    ...
    assert result.transferred_types_by_level == (53,)
    assert result.occurrences_by_level == (92,)
    assert result.covered_atoms_by_level == (560,)
    assert result.residual_atoms_by_level == (399,)
    assert result.atom_coverage_by_level == (560 / 959,)
    assert result.minimum_namespaces_by_level == (0,)
    assert result.minimum_independent_occurrences_by_level == (0,)
