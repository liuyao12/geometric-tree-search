...
def test_attempt_receipt_binds_the_committed_protocol_and_center():
    ...
    assert not Path(DEFAULT_FIXTURE).exists()
    assert not Path(ATTEMPT_MARKER).exists()
    assert CONFIRMATION_CENTER == (360., 0., 0.)


if __name__ == "__main__":
    test_attempt_receipt_binds_the_committed_protocol_and_center()
