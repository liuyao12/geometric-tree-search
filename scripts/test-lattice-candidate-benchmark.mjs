...
assert.equal(survivor.schemaVersion, 22);
...
const coldGctsRows = survivor.rows.filter(row => row.lane === "gcts");
assert.ok(coldGctsRows.length > 0);
assert.ok(coldGctsRows.every(row => row.moveOrder === "balanced"));
assert.ok(coldGctsRows.every(row => row.gctsFailureMarkingEnabled));
assert.ok(coldGctsRows.every(row => row.markingStartedEmpty));
assert.ok(coldGctsRows.every(row => row.learnedProgram === null && !row.reusedLearnedPatch));
...
assert.ok(memoRow.failureMemoHits >= 8, "candidate 10_45026 must exercise repeated duplicate-state reuse");
