...
def test_transport_and_receipt_boundaries_are_visible():
    for fragment in (
        ...
        assert fragment in APP
    assert "Build 341 · encode a validated trajectory as local path geometry" in README
    assert "Validated trajectory-to-section deployment (Build 341)" in BENCHMARK


def test_build_341_is_cache_busted():
    for document in (HTML, ALIAS_HTML):
        assert 'app.js?v=20260830-341' in document
        assert 'style.css?v=20260830-341' in document
        assert 'evidence-atlas.js?v=20260830-341' in document
    assert 'buildId: "20260830-341"' in APP
...
def main() -> None:
    ...
    test_build_341_is_cache_busted()
