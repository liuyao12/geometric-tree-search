...
assert.equal(report.siteRoles.length, 6);
assert.deepEqual(report.siteRoles.filter((site) => site.poseInterface).map((site) => site.index), [0, 1, 2, 3]);
assert.deepEqual(report.siteRoles.filter((site) => site.gapBoundary).map((site) => site.index), [3, 4]);
assert.deepEqual(report.siteRoles.filter((site) => site.literalTerminal).map((site) => site.index), [5]);
assert.deepEqual(report.siteRoles.filter((site) => site.coordinationAnomaly).map((site) => site.index), [3]);
assert.deepEqual(report.siteRoles.filter((site) => site.occupationalAlternative).map((site) => site.index), [4, 5]);
assert.deepEqual(report.siteRoles.filter((site) => site.explicitVacancy).map((site) => site.index), [5]);
