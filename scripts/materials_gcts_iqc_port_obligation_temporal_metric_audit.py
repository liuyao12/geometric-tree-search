#!/usr/bin/env python3
"""Grouped audit of an order-sensitive IQC port-obligation metric.

Geometry-only temporal neighbor receipts are frozen before any label is read.
Whole-nucleus selection and all 31 within-nucleus label shuffles repeat the
same model-selection procedure.  Only after a final train-only model ranks the
target-free consumed trajectories do we join the published partial ordering;
the confirmation target is neither reconstructed nor reopened.
"""
...
from materials_gcts_iqc_port_obligation_role_metric_audit import (
    _action_tuple, _auc, _candidate_id, _shuffle)
from materials_gcts_iqc_relational_port_rule import SHUFFLES
...
NEIGHBORS = (3, 5, 7, 9)
WEIGHTED = (False, True)
SPECS = tuple(PortObligationTemporalMetricSpec(*values)
              for values in itertools.product(
                  HORIZONS, TIME_BINS, SEPARATION_CHANNELS,
                  NEIGHBORS, WEIGHTED))
...
def _feature_key(spec):
    return spec.horizon, spec.time_bins, spec.separation_channels


def _feature_spec(key):
    return PortObligationTemporalMetricSpec(*key, 1, False)


def _scales(vectors, indices):
    means = tuple(sum(vectors[index][dimension] for index in indices) /
                  len(indices) for dimension in range(len(vectors[0])))
    return tuple(max(1e-9, math.sqrt(sum(
        (vectors[index][dimension] - means[dimension]) ** 2
        for index in indices) / len(indices)))
        for dimension in range(len(means)))
...
def _freeze_neighbor_receipts(geometry):
    ...
    receipts = {}
    thresholds = {}
    keys = tuple(sorted({_feature_key(spec) for spec in SPECS}))
...
def _heldout(geometry, labels, sites, spec, receipts):
    ...
    epsilon = 1e-9
    logloss = -sum(float(labels[candidate]) * math.log(max(epsilon, score)) +
                   (1. - float(labels[candidate])) * math.log(max(
                       epsilon, 1. - score))
                   for candidate, score in scores.items()) / len(scores)
    return {
...
def _objective(result, index):
    ...
    return (result["exact"], result["sites"], -result["logloss"],
            result["auc"], -spec.horizon, -spec.time_bins,
            not spec.separation_channels, -spec.neighbors,
            spec.weighted, -index)
...
def evaluate():
    ...
    sites = {_candidate_id(row): int(raw_labels[
        ...
        for row in geometry}
    receipts, receipt_digest = _freeze_neighbor_receipts(geometry)
    selected_index, audits = _select(geometry, labels, sites, receipts)
    ...
    model = fit_port_obligation_temporal_metric(
        training_rows, selected_spec)
    external = load_confirmation_trajectories()
    scored = tuple((row, score_port_obligation_temporal_metric(
    ...
    frozen_scores = tuple(score for _row, score in ranked)
    ...
    body = {
        ...
        "known_exact_temporal_metric_rank": known_exact_rank,
        "known_false_temporal_metric_ranks": false_ranks,
    ...
    body["temporal_metric_diagnostic_passed"] = bool(
...
def main():
    ...
    print(json.dumps(report, indent=2, sort_keys=True) if args.json else
          ("temporal obligation metric diagnostic passes" if
           report["temporal_metric_diagnostic_passed"] else
           "temporal obligation metric remains exploratory"))
