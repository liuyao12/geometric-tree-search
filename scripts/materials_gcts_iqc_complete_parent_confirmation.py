#!/usr/bin/env python3
"""One-shot fresh confirmation for the complete-parent IQC successor."""
...
def _execute_and_score_once():
    ...
    execution = freeze_three_block_complete_parent_execution(
    ...
    if (execution.target_used or
            len(execution.selected_parent_ids) != COMPLETE_PARENT_WIDTH or
            len(set(execution.selected_parent_ids)) != COMPLETE_PARENT_WIDTH or
        ...
        raise AssertionError("complete-parent receipt invariant drift")
    ...
    seed_subset = set(seed_lifts).issubset(target_lifts)
    success = bool(
        ...
        set(execution.selected_parent_ids) == set(range(1, 9)))
    body = {
        "schema_version": 1,
        ...
        "fresh_complete_parent_three_block_supply_confirmed": success,
...
def validate_result(row):
    ...
    if (hashlib.sha256(canonical_json(body)).hexdigest() != digest or
            ...
            len(body["receipt"]["selected_parent_ids"]) != 8 or
        ...
        raise AssertionError("complete-parent fresh result drift")
    if EXPECTED_RESULT_DIGEST and digest != EXPECTED_RESULT_DIGEST:
        raise AssertionError("complete-parent result digest drift")
    return row
...
def load_default_result(path=DEFAULT_FIXTURE):
    ...
    if (EXPECTED_FIXTURE_SHA256 and
        ...
        raise AssertionError("complete-parent fixture byte drift")
    return validate_result(json.loads(gzip.decompress(raw)))


def _attempt_marker_bytes():
    return canonical_json({
        "schema_version": 1, "protocol_digest": EXPECTED_MANIFEST_DIGEST,
...
def _summary(row):
    return {key: row[key] for key in (
        "fresh_complete_parent_three_block_supply_confirmed", "seed_atoms",
        "target_atoms", "candidate_lineages", "exact_first_prefixes",
        "exact_second_prefixes", "best_correct_actions", "exact_lineages",
        "exact_parent_child_ids", "receipt_digest", "result_digest")}
...
def main():
    ...
    if args.write:
        if DEFAULT_FIXTURE.exists() or ATTEMPT_MARKER.exists():
            raise RuntimeError("complete-parent one-shot already consumed")
