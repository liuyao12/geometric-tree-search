...
const shared = points.filter(p => p.markings.size === 2); assert(shared.length > 0);
for (const p of shared) {
  ...
  assert.equal(inspectionText(p, true).m, `m(x) = (${a.value.join(', ')}) · 2 tiles agree`, 'do not sum matching m values');
}
