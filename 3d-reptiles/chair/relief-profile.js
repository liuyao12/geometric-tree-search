...
export function reliefFeatures(color, centered = false) {
  // Touching bases join the inward slopes into one coplanar blue face.
  if (color === 'blue') return [
    { u: .16, v: centered ? 0 : .08, radius: .16, height: .12 },
    { u: -.16, v: centered ? 0 : .08, radius: .16, height: -.12 },
  ];
  return [{ u: 0, v: centered ? 0 : .10, radius: .24, height: color === 'red' ? .12 : -.12 }];
}
...
export function reliefHeight(mark, point, centered = false) {
  ...
  return reliefFeatures(mark.color, centered).reduce((sum, feature) => sum + feature.height * Math.max(0,
