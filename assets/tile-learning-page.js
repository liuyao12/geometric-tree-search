...
const reports=new Map(),recordedReports=new Map();
...
function marks(entries,map){for(const e of entries){const q=map(e.point),d=[1,1,1];d[e.component]=-2;const tip=map(e.point.map((v,i)=>v+.25*d[i]));ctx.strokeStyle=e.value===0?'#acb9b355':`hsl(${Math.abs(e.value)*137.5%360} 60% 37%)`;ctx.lineWidth=e.value===0?.6:1.7;ctx.setLineDash(e.value<0?[2,2]:[]);ctx.beginPath();ctx.moveTo(...q);ctx.lineTo(...tip);ctx.stroke();}ctx.setLineDash([]);}
function draw(){ctx.fillStyle='#fafbf7';ctx.fillRect(0,0,720,360);ctx.fillStyle='#294d43';ctx.font='14px system-ui';ctx.fillText(mode?'Searching local connections':'Connection & extension',15,23);ctx.fillText('Learned point codes',390,23);
 const specs=shown.length?shown:[learner.roots[0]],placements=specs.map(learner.materialize),map=mapFor(placements.flatMap(p=>p.loop),{x:0,y:35,w:370,h:315});
 for(const [i,p] of placements.entries()){polygon(p.loop,map,p.tile==='hat'?'#ecd1ad':'#bad9d2');if(model&&$('learn-marks').checked)marks(learner.entries(specs[i],model.support),map);}
 ...
 for(const [i,tile] of learner.config.tiles.entries()){const root=learner.roots[i],p=learner.materialize(root),n=learner.config.tiles.length,box={x:390,y:32+i*310/n,w:310,h:310/n},m=mapFor([...p.loop,...learner.pointDomain(tile)],box);polygon(p.loop,m,tile==='hat'?'#ecd1ad':'#bad9d2');if(model&&$('learn-marks').checked)marks(model.support.filter(e=>e.tile===tile),m);ctx.fillStyle='#486656';ctx.font='12px system-ui';ctx.fillText(tile,box.x,box.y+18);}
 ctx.strokeStyle='#e1e7df';ctx.beginPath();ctx.moveTo(380,35);ctx.lineTo(380,348);ctx.stroke();
}
function finish(){worker?.terminate();worker=null;mode=null;paused=false;$('learn-start').disabled=false;$('learn-start').textContent='Learn markings';$('learn-deeper').textContent='Search farther';picker.disabled=!report;$('learn-apply').disabled=!model;$('learn-export').disabled=!report;$('learn-deeper').disabled=!report||report.connections[+picker.value]?.status==='dead';}
...
function install(data,recorded=false,index=0){if(data.setId!==setId)return;report=data;model=data.model;if(model)learner.validateModel(model);reports.set(setId,data);recordedReports.set(setId,recorded);picker.replaceChildren(...data.connections.map((row,i)=>{const o=document.createElement('option');o.value=i;o.textContent=`${i+1} · ${row.root.tile}–${row.attachment.tile} · ${row.status==='extended'?`${row.placements.length} tiles`:row.status}`;return o;}));picker.value=index;shown=data.connections[index]?.placements||[];
 ...
 finish();describe();draw();if(model)send('gcts-marking-ready',{model,counts:data.counts,recorded});
}
function reset(){epoch++;finish();report=null;model=null;shown=[];picker.replaceChildren();picker.disabled=true;$('learn-deeper').disabled=true;$('learn-apply').disabled=true;$('learn-export').disabled=true;$('learn-metrics').textContent=TILE_SETS[setId].label;message('Examine every legal connection; use all extension and failure evidence.');draw();}
...
function launch(kind){epoch++;worker?.terminate();worker=new Worker(new URL('./tile-learning-worker.js?v=20260915-1',import.meta.url),{type:'module'});const active=worker;mode=kind;paused=false;
 $('learn-start').disabled=kind!=='collect';$('learn-deeper').disabled=kind!=='extend';$('learn-apply').disabled=true;$('learn-export').disabled=true;picker.disabled=true;
...
$('learn-apply').onclick=()=>{if(!model)return;send('gcts-use-marking',{model,counts:report.counts,recorded:recordedReports.get(setId)});if(!embedded){sessionStorage.setItem('gcts-requested-marking',JSON.stringify(model));location.href='GCTS-I.html#anchor-learning';}};
$('learn-export').onclick=()=>{const url=URL.createObjectURL(new Blob([JSON.stringify(report,null,2)],{type:'application/json'})),a=document.createElement('a');a.href=url;a.download=`${setId}-connection-evidence.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
window.addEventListener('message',e=>{if(e.origin===location.origin&&e.source===parent&&e.data?.type==='gcts-select-tiles'&&e.data.setId!==setId)choose(e.data.setId);});
