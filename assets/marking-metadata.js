...
export function markingMetadata(model){
 ...
 return {lattice:model?.lattice??'A2',points:perTile.reduce((n,t)=>n+t.points,0),values:support.length,nonzero:support.filter(e=>e.value!==0).length,distinctValues:new Set(support.map(e=>e.value)).size,perTile};
}
